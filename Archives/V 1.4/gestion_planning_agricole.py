...
class PlanningApp(tk.Tk):
    ...
    def _build_widgets(self):
        ...
        onglet_historique = ttk.Frame(notebook)
        onglet_fiches = ttk.Frame(notebook)
        ...
        notebook.add(onglet_historique, text="Historique météo")
        notebook.add(onglet_fiches, text="Fiches de référence")
    ...
    def _build_onglet_historique(self, parent):
        ...
        ttk.Label(form, text="Ville (Météociel) :").grid(row=0, column=0, sticky="w", padx=4, pady=4)
        self.var_hist_ville = tk.StringVar(value="Paris (75000)")
        ttk.Entry(form, textvariable=self.var_hist_ville, width=22).grid(
            row=0, column=1, padx=4, pady=4, sticky="w")

        hier = datetime.date.today() - datetime.timedelta(days=1)
        debut_defaut = hier - datetime.timedelta(days=6)

        ttk.Label(form, text="Du :").grid(row=0, column=2, sticky="e", padx=4, pady=4)
        self.var_hist_j1 = tk.StringVar(value=str(debut_defaut.day))
        self.var_hist_m1 = tk.StringVar(value=str(debut_defaut.month))
        self.var_hist_a1 = tk.StringVar(value=str(debut_defaut.year))
        ttk.Spinbox(form, from_=1, to=31, width=3, textvariable=self.var_hist_j1).grid(row=0, column=3)
        ttk.Spinbox(form, from_=1, to=12, width=3, textvariable=self.var_hist_m1).grid(row=0, column=4)
        ttk.Spinbox(form, from_=2000, to=2100, width=5, textvariable=self.var_hist_a1).grid(row=0, column=5)

        ttk.Label(form, text="au :").grid(row=0, column=6, sticky="e", padx=4, pady=4)
        self.var_hist_j2 = tk.StringVar(value=str(hier.day))
        self.var_hist_m2 = tk.StringVar(value=str(hier.month))
        self.var_hist_a2 = tk.StringVar(value=str(hier.year))
        ttk.Spinbox(form, from_=1, to=31, width=3, textvariable=self.var_hist_j2).grid(row=0, column=7)
        ttk.Spinbox(form, from_=1, to=12, width=3, textvariable=self.var_hist_m2).grid(row=0, column=8)
        ttk.Spinbox(form, from_=2000, to=2100, width=5, textvariable=self.var_hist_a2).grid(row=0, column=9)

        self.btn_hist_base = ttk.Button(form, text="Générer la base de villes (une fois)",
                                         command=self._lancer_generation_base_villes)
        self.btn_hist_base.grid(row=1, column=0, columnspan=4, pady=(8, 4), sticky="ew")

        self.btn_hist_charger = ttk.Button(form, text="Charger l'historique",
                                            command=self._lancer_chargement_historique)
        self.btn_hist_charger.grid(row=1, column=4, columnspan=3, pady=(8, 4), sticky="ew")

        self.lbl_hist_status = ttk.Label(parent, text="", foreground="#555")
        self.lbl_hist_status.pack(side="top", fill="x", padx=14)

        self.lbl_hist_valeur = ttk.Label(
            parent, text="Cliquez sur un point du graphique pour afficher sa valeur exacte.",
            foreground="#0b5a9e", font=("TkDefaultFont", 10, "bold"))
        self.lbl_hist_valeur.pack(side="top", fill="x", padx=14, pady=(4, 0))
        ...
        toolbar_frame = ttk.Frame(graph_frame)
        toolbar_frame.pack(side="top", fill="x")
        NavigationToolbar2Tk(self.canvas_hist, toolbar_frame).update()
    ...
    def _lire_dates_historique(self):
        try:
            d1 = datetime.date(int(self.var_hist_a1.get()), int(self.var_hist_m1.get()), int(self.var_hist_j1.get()))
            d2 = datetime.date(int(self.var_hist_a2.get()), int(self.var_hist_m2.get()), int(self.var_hist_j2.get()))
        except ValueError:
            messagebox.showwarning("Date invalide", "Veuillez saisir des dates valides.")
            return None, None
        return d1, d2

    def _lancer_generation_base_villes(self):
        if md.base_villes_disponible():
            messagebox.showinfo("Info", "La base de villes est déjà disponible sur cette machine.")
            return
        if not messagebox.askyesno(
            ...
            return
        self.btn_hist_base.configure(state="disabled")
        self.lbl_hist_status.configure(text="⏳ Génération de la base de villes en cours...")

        def tache_fond():
            try:
                md.generer_base_villes()
                erreur = None
            except Exception as e:
                erreur = e
            self.after(0, lambda: self._generation_base_terminee(erreur))

        threading.Thread(target=tache_fond, daemon=True).start()

    def _generation_base_terminee(self, erreur):
        self.btn_hist_base.configure(state="normal")
        if erreur is not None:
            self.lbl_hist_status.configure(text=f"❌ Échec de la génération : {erreur}")
            return
        self.lbl_hist_status.configure(text="✅ Base de villes générée avec succès.")

    def _lancer_chargement_historique(self):
        ville = self.var_hist_ville.get().strip()
        if not ville:
            messagebox.showwarning("Champ requis", "Veuillez indiquer une ville.")
            return
        d1, d2 = self._lire_dates_historique()
        if d1 is None:
            return
        nb_jours = abs((d2 - d1).days) + 1
        if nb_jours > 31 and not messagebox.askyesno(
                "Période longue",
                f"La période sélectionnée représente {nb_jours} jours, donc {nb_jours} appels "
                "réseau successifs. Cela peut être long. Continuer ?"):
            return

        self.btn_hist_charger.configure(state="disabled")
        self.lbl_hist_status.configure(text="⏳ Récupération de l'historique en cours...")

        def tache_fond():
            try:
                ville_trouvee, df = md.recuperer_historique(ville, d1, d2)
                erreur = None
            except Exception as e:
                ville_trouvee, df, erreur = None, None, e
            self.after(0, lambda: self._historique_termine(ville_trouvee, df, erreur))

        threading.Thread(target=tache_fond, daemon=True).start()

    def _historique_termine(self, ville_trouvee, df, erreur):
        self.btn_hist_charger.configure(state="normal")
        if erreur is not None:
            self.lbl_hist_status.configure(text=f"❌ Échec de la récupération : {erreur}")
            return
        if df is None or df.empty:
            self.lbl_hist_status.configure(text="Aucune donnée renvoyée par Météociel pour cette période.")
            return
        ...
        self.lbl_hist_status.configure(
            text=f"✅ Historique chargé pour « {ville_trouvee} » ({len(df)} relevés).")
        self.lbl_hist_valeur.configure(
            text="Cliquez sur un point du graphique pour afficher sa valeur exacte.")
    ...
    def _clic_graphique_historique(self, event):
        ...
        self.lbl_hist_valeur.configure(text=texte)
    ...
    def _build_onglet_fiches(self, parent):
        gauche = ttk.Frame(parent, padding=8)
        gauche.pack(side="left", fill="y")

        ttk.Label(gauche, text="Rechercher :").pack(anchor="w")
        self.var_fiche_recherche = tk.StringVar()
        self.var_fiche_recherche.trace_add("write", lambda *a: self._filtrer_liste_fiches())
        ttk.Entry(gauche, textvariable=self.var_fiche_recherche, width=26).pack(fill="x", pady=(0, 6))

        liste_frame = ttk.Frame(gauche)
        liste_frame.pack(fill="both", expand=True)
        self.liste_fiches = tk.Listbox(liste_frame, width=28, exportselection=False)
        self.liste_fiches.pack(side="left", fill="both", expand=True)
        scroll_fiches = ttk.Scrollbar(liste_frame, orient="vertical", command=self.liste_fiches.yview)
        scroll_fiches.pack(side="left", fill="y")
        self.liste_fiches.configure(yscrollcommand=scroll_fiches.set)
        self.liste_fiches.bind("<<ListboxSelect>>", lambda e: self._afficher_fiche_selection())

        nav = ttk.Frame(gauche)
        nav.pack(fill="x", pady=(6, 0))
        ttk.Button(nav, text="◀ Précédent", command=lambda: self._naviguer_fiche(-1)).pack(
            side="left", expand=True, fill="x")
        ttk.Button(nav, text="Suivant ▶", command=lambda: self._naviguer_fiche(1)).pack(
            side="left", expand=True, fill="x")

        droite = ttk.Frame(parent, padding=10)
        droite.pack(side="left", fill="both", expand=True)

        self.lbl_fiche_titre = ttk.Label(droite, text="Sélectionnez une culture",
                                          font=("TkDefaultFont", 18, "bold"))
        self.lbl_fiche_titre.pack(anchor="w")

        self.lbl_fiche_sous_titre = ttk.Label(droite, text="", foreground="#555")
        self.lbl_fiche_sous_titre.pack(anchor="w", pady=(0, 8))

        colonnes = ("conduite", "variete_n", "action", "semaines", "mois", "commentaire")
        entetes = ("Conduite", "Variété n°", "Action", "Semaines", "Mois", "Commentaire")
        self.tree_fiche = ttk.Treeview(droite, columns=colonnes, show="headings", height=16)
        for col, ent in zip(colonnes, entetes):
            self.tree_fiche.heading(col, text=ent)
            self.tree_fiche.column(col, width=230 if col == "commentaire" else 100, anchor="w")
        self.tree_fiche.pack(fill="both", expand=True)
    ...
    def _filtrer_liste_fiches(self):
        texte = self.var_fiche_recherche.get().strip().lower() if hasattr(self, "var_fiche_recherche") else ""
        cultures = getattr(self, "_toutes_cultures_fiches", [])
        if texte:
            cultures = [c for c in cultures if texte in c.lower()]
        self.liste_fiches.delete(0, "end")
        for c in cultures:
            self.liste_fiches.insert("end", c)

    def _naviguer_fiche(self, pas: int):
        taille = self.liste_fiches.size()
        if taille == 0:
            return
        sel = self.liste_fiches.curselection()
        idx = (sel[0] + pas) if sel else 0
        idx = max(0, min(taille - 1, idx))
        self.liste_fiches.selection_clear(0, "end")
        self.liste_fiches.selection_set(idx)
        self.liste_fiches.see(idx)
        self._afficher_fiche_selection()

    def _afficher_fiche_selection(self):
        sel = self.liste_fiches.curselection()
        if not sel:
            return
        culture = self.liste_fiches.get(sel[0])
        lignes = [r for r in self.rows if r.get("culture") == culture]

        emoji = self._emoji_pour_culture(culture)
        self.lbl_fiche_titre.configure(text=f"{emoji}  {culture}")

        conduites = sorted({r["conduite"] for r in lignes})
        varietes = sorted({str(r.get("variete_n", "1")) for r in lignes}, key=lambda x: (len(x), x))
        self.lbl_fiche_sous_titre.configure(
            text=f"Conduite(s) : {', '.join(conduites)}  •  {len(varietes)} variété(s)/étalement(s) référencé(s)")

        self.tree_fiche.delete(*self.tree_fiche.get_children())
        ...
        for r in sorted(lignes, key=cle_tri):
            semaines = f"{r.get('semaine_debut', '')}-{r.get('semaine_fin', '')}"
            mois = f"{r.get('mois_debut', '')} - {r.get('mois_fin', '')}"
            self.tree_fiche.insert("", "end", values=(
                r.get("conduite", ""), r.get("variete_n", ""), r.get("action", ""),
