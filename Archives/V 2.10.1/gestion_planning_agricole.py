...
class PlanningApp(QMainWindow):
    ...
    def _build_onglet_carte(self, parent):
        ...
        intro = QLabel(
            ...
            "hexagonales de même taille) ou géographique (contours réels, données IGN). "
            ...
            "Données à l'échelle régionale, donc volontairement simplifiées - la couche "
            "« sol » n'est pas une carte géologique BRGM, mais une appréciation indicative "
            "de fertilité dominante. Pour une décision réelle, reportez-vous à la météo "
            "locale (onglet « Aide à la décision ») et à une analyse de sol de votre "
            "parcelle (onglet « Analyse de sol »)."
        )
        intro.setWordWrap(True)
        intro.setStyleSheet(f"color: {COULEUR_INFO}; font-style: italic; font-size: 9pt;")
        layout.addWidget(intro)
