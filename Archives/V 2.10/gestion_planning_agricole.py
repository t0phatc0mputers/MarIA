...
class PlanningApp(QMainWindow):
    ...
    def _build_onglet_carte(self, parent):
        ...
        intro = QLabel(
            "Carte schématique (en tuiles) des 13 régions de France métropolitaine. "
            "Cliquez sur une région pour afficher son climat et la qualité indicative de "
            "son sol ; molette pour zoomer/dézoomer, clic-glisser pour déplacer la carte. "
            "Données à l'échelle régionale, donc volontairement simplifiées : pour une "
        ...
        barre = QHBoxLayout()
        barre.addWidget(QLabel("Affichage :"))
        self.carte_combo_mode = QComboBox()
        self.carte_combo_mode.addItems(cf.MODES)
        self.carte_combo_mode.currentIndexChanged.connect(self._carte_rafraichir_couleurs)
        barre.addWidget(self.carte_combo_mode)
        barre.addStretch(1)
        btn_carte_zoom = QPushButton("🔍 Réinitialiser le zoom")
        btn_carte_zoom.clicked.connect(lambda: self.carte_vue.ajuster_vue())
        barre.addWidget(btn_carte_zoom)
        layout.addLayout(barre)
        ...
        self.carte_tuiles = {}
        for nom, d in cf.REGIONS.items():
            cx, cy = cf.position_pixel(d["row"], d["col"])
            tuile = cf.TuileRegion(nom, cx, cy, callback_clic=self._carte_selectionner)
            self.carte_scene.addItem(tuile)
            self.carte_scene.addItem(cf.creer_etiquette(nom, cx, cy))
            self.carte_tuiles[nom] = tuile

        self.carte_scene.setSceneRect(self.carte_scene.itemsBoundingRect().adjusted(-40, -40, 40, 40))

        self._carte_rafraichir_couleurs()
        region_defaut = "Île-de-France" if "Île-de-France" in cf.REGIONS else next(iter(cf.REGIONS))
        self._carte_selectionner(region_defaut)

    def _carte_rafraichir_couleurs(self):
        mode_index = self.carte_combo_mode.currentIndex()
        for nom, tuile in self.carte_tuiles.items():
            tuile.definir_couleur(cf.couleur_pour(nom, mode_index))
    ...
    def _carte_selectionner(self, nom):
        for autre, tuile in self.carte_tuiles.items():
