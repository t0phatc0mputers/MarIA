...
def _resoudre_id_previsions(nom_recherche, mode, modele):
    """Résout l'identifiant de prévisions Météociel correspondant à
    ``nom_recherche``. Renvoie l'identifiant (str).
    ...
    correspondent encore (aucune n'est un nom exact), ``ValueError`` si
    aucune ville n'est trouvée, ``ConnectionError`` en cas de problème
    réseau - comme la fonction d'origine."""
    try:
    ...
    for identifiant, libelle in candidats:
        if libelle.lower() == nom_recherche.strip().lower():
            return identifiant
...
def _resoudre_et_appeler_previsions(ville, ville_id, mode, modele):
    ...
    # et son code postal quand il est connu, au format "Nom (code postal)"
    # attendu par Météociel en cas d'ambiguïté - plus fiable qu'un simple
    # nom tapé à la main.
    nom_recherche = ville
    if ville_id:
        nom_reel = gs.nom_pour_id(ville_id)
        if nom_reel:
            code_postal = gs.code_postal_pour_id(ville_id)
            nom_recherche = f"{nom_reel} ({code_postal})" if code_postal else nom_reel

    id_trouve = _resoudre_id_previsions(nom_recherche, mode, modele)
...
def _station_par_id(date: datetime.datetime, city_id: str):
    ...
    wind_spd, wind_gust = _station_conv(data[-3][1:])
