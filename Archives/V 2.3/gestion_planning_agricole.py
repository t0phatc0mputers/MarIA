...
from PyQt5.QtWidgets import (
    ...
    QScrollArea, QFrame, QSizePolicy, QDoubleSpinBox, QProgressBar,
)
...
class PlanningApp(QMainWindow):
    ...
    def _build_onglet_qualite_sol(self, parent):
        ...
        self.cb_sol_mo = QComboBox()
        self.cb_sol_mo.addItems(qs.CHOIX_MATIERE_ORGANIQUE)
        form_sol.addRow("Matière organique :", self.cb_sol_mo)
        ...
        box_chimie = QGroupBox("⚗️ Concentration des éléments (optionnel)")
        form_chimie = QFormLayout(box_chimie)

        self.cb_sol_azote = QComboBox()
        self.cb_sol_azote.addItems(qs.CHOIX_NIVEAU)
        form_chimie.addRow("Azote (N) :", self.cb_sol_azote)

        self.cb_sol_phosphore = QComboBox()
        self.cb_sol_phosphore.addItems(qs.CHOIX_NIVEAU)
        form_chimie.addRow("Phosphore (P) :", self.cb_sol_phosphore)

        self.cb_sol_potassium = QComboBox()
        self.cb_sol_potassium.addItems(qs.CHOIX_NIVEAU)
        form_chimie.addRow("Potassium (K) :", self.cb_sol_potassium)

        self.cb_sol_salinite = QComboBox()
        self.cb_sol_salinite.addItems(qs.CHOIX_SALINITE)
        form_chimie.addRow("Salinité :", self.cb_sol_salinite)
    ...
    def _lire_profil_sol(self):
        ...
        return {
            ...
            "matiere_organique": self.cb_sol_mo.currentText(),
            ...
            "azote": self.cb_sol_azote.currentText(),
            "phosphore": self.cb_sol_phosphore.currentText(),
            "potassium": self.cb_sol_potassium.currentText(),
            "salinite": self.cb_sol_salinite.currentText(),
        }

    def _reinitialiser_sol(self):
        self.spin_sol_ph.setValue(0.0)
        for combo in (self.cb_sol_texture, self.cb_sol_mo, self.cb_sol_humidite,
                      self.cb_sol_drainage, self.cb_sol_calcaire, self.cb_sol_azote,
                      self.cb_sol_phosphore, self.cb_sol_potassium, self.cb_sol_salinite):
