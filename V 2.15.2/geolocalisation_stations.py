...
def codes_postaux_valides():
    """Ensemble de tous les codes postaux français connus (valeurs de
    codes_postaux_insee.json). Utilisé pour distinguer, dans une liste de
    candidats Météociel (recherche sur le site des PRÉVISIONS, qui n'est
    pas limitée à la France), les véritables communes françaises des
    homonymes étrangers - ex. une recherche "Bage" renvoie aussi bien
    "Bâgé-la-Ville (01380)" que "Bagé (Brésil)" ou "Bagepalli (IN)" ; seul
    le premier a un code postal présent dans cet ensemble.

    Attention : quelques codes postaux étrangers (Belgique, Suisse...) sont
    eux aussi purement numériques et pourraient coïncider par hasard avec
    un code postal français existant - ce filtre réduit donc fortement,
    sans l'éliminer totalement à 100 %, le risque de confusion avec un
    homonyme étranger."""
    return set(_charger_codes_postaux().values())
