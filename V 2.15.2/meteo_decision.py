...
def _ne_garder_que_france(candidats):
    """Filtre ``candidats`` (voir ``_resoudre_id_previsions``) pour ne
    garder que les communes françaises, identifiées par un code postal
    entre parenthèses qui correspond à un code postal français connu (voir
    ``geolocalisation_stations.codes_postaux_valides``).

    Une recherche par nom sur le site des PRÉVISIONS Météociel n'est PAS
    limitée à la France (contrairement à ``geolocalisation_stations.
    rechercher_villes``, restreinte via ``pays="france"``) : une simple
    recherche comme "Bage" renvoie donc, pêle-mêle, des communes françaises
    ("Bâgé-la-Ville (01380)") et des homonymes étrangers ("Bagé (Brésil)",
    "Bagepalli (IN)", "Bageshwar (IN)") - ces derniers gonflaient
    artificiellement le nombre d'ambiguïtés remontées, alors qu'aucun
    utilisateur de cette application (dédiée au maraîchage en Île-de-France)
    ne recherche une ville hors de France.

    Si aucun candidat ne passe ce filtre (ex. base des codes postaux
    incomplète, ou candidats déjà uniquement français mais dont le libellé
    ne porte exceptionnellement pas de code postal), la liste d'origine est
    renvoyée inchangée plutôt que de tout écarter à tort."""
    codes_valides = gs.codes_postaux_valides()
    filtres = [(i, l) for i, l in candidats if _extraire_code_postal(l) in codes_valides]
...
def _choisir_parmi_candidats(candidats, nom_recherche, code_postal_attendu):
    ...
    0. Les candidats qui ne sont pas des communes françaises (homonymes
       étrangers - voir ``_ne_garder_que_france``) sont écartés d'emblée :
       cette application ne concerne que des villes françaises, et ces
       homonymes ne font qu'ajouter du bruit à la désambiguïsation.
    ...
    3. Sinon, si un seul candidat (français) existe au total, il est renvoyé.
    4. Sinon, lève ``TooManyCitiesError`` avec la liste lisible des
       candidats (français uniquement), pour affichage à l'utilisateur -
       ambiguïté réelle entre plusieurs communes françaises homonymes (ex.
       deux communes fusionnées partageant le même code postal)."""
...
def _resoudre_id_previsions(nom_recherche, mode, modele, code_postal_attendu=None):
    ...
    bien pour une recherche qui correspond à une seule ville - le cas le
    plus fréquent une fois qu'on recherche par "Nom (code postal)"), puis,
    si elle échoue avec ce ``AttributeError`` précis OU si elle lève
    elle-même proprement ``TooManyCitiesError`` (cas où son sélecteur
    fonctionne mais où plusieurs villes correspondent encore : ex. "Balan"
    correspond à deux communes françaises distinctes, Ain et Ardennes, en
    plus d'homonymes étrangers), on refait la même requête HTTP et on
    extrait nous-mêmes les villes candidates par expression régulière
    directement dans la page (même motif que celui utilisé en interne par
    le paquet), sans dépendre du sélecteur de tableau cassé - ce qui permet
    ensuite d'appliquer notre propre filtrage France + désambiguïsation par
    code postal (voir ``_choisir_parmi_candidats``), que le paquet lui-même
    ne fait pas.
    ...
    except _TooManyCitiesError:
        pass  # ambiguïté détectée par le paquet lui-même, mais avec sa propre liste brute
              # (non filtrée France, pas de désambiguïsation par code postal) : on repasse
              # par le repli manuel ci-dessous pour bénéficier de _choisir_parmi_candidats
