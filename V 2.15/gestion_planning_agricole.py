...
class PlanningApp(QMainWindow):
    ...
    def _build_onglet_decision(self, parent):
        ...
        self.edit_dec_ville.textEdited.connect(lambda: self.edit_dec_ville.setProperty("meteociel_id", None))
    ...
    def _lancer_analyse_decision(self):
        ...
        ville_id = self._id_ville(self.edit_dec_ville)
        mode = md.MODES_DISPONIBLES.get(self.cb_dec_mode.currentText(), "forecasts")
        ...
        thread = WorkerThread(md.recommander, action, ville, mode=mode, modele=modele, ville_id=ville_id)
    ...
    def _analyse_terminee(self, texte_calendrier, resultat, erreur):
        ...
        if isinstance(erreur, md.PlusieursVillesTrouvees):
            def annuler():
                self.txt_dec_resultat.setPlainText(
                    texte_calendrier +
                    "\n\n❌ Analyse annulée : plusieurs villes correspondaient à la recherche.")
                self._set_status("Analyse météo annulée.")
            self._proposer_choix_villes(
                erreur.candidats, self.edit_dec_ville,
                self._lancer_analyse_decision, annuler)
            return
    ...
    def _build_onglet_graphiques(self, parent):
        ...
        self.edit_graph_ville.textEdited.connect(lambda: self.edit_graph_ville.setProperty("meteociel_id", None))
    ...
    def _lancer_chargement_graphique(self):
        ...
        ville_id = self._id_ville(self.edit_graph_ville)
        if not ville:
            QMessageBox.warning(self, "Champ requis", "Veuillez indiquer une ville pour la météo.")
            return
        mode = md.MODES_DISPONIBLES.get(self.cb_graph_mode.currentText(), "forecasts")
        ...
        thread = WorkerThread(md.recuperer_previsions, ville, mode=mode, modele=modele, ville_id=ville_id)
    ...
    def _graphique_termine(self, ville_trouvee, df, erreur):
        ...
        if isinstance(erreur, md.PlusieursVillesTrouvees):
            def annuler():
                self.lbl_graph_status.setText(
                    "Chargement annulé : plusieurs villes correspondaient à la recherche.")
                self.lbl_graph_status.setStyleSheet(f"color: {COULEUR_ALERTE};")
            self._proposer_choix_villes(
                erreur.candidats, self.edit_graph_ville,
                self._lancer_chargement_graphique, annuler)
            return
        if erreur is not None:
    ...
    def _build_onglet_historique(self, parent):
        ...
        self.edit_hist_ville.textEdited.connect(lambda: self.edit_hist_ville.setProperty("meteociel_id", None))
    ...
    def _choisir_ville(self, champ_ville):
        """Ouvre la pop-up « Choisir une localisation » (recherche par nom,
        carte cliquable ou coordonnées GPS) et, si l'utilisateur valide un
        choix, met à jour le champ de saisie 'ville' Météociel fourni ainsi
        que l'identifiant associé (voir _id_ville / meteo_decision.py,
        paramètre ``ville_id``), à utiliser en priorité pour interroger
        Météociel de façon fiable."""
        resultat = dl.choisir_localisation(self)
        if resultat:
            champ_ville.setText(resultat["nom"])
            champ_ville.setProperty("meteociel_id", resultat.get("id"))

    @staticmethod
    def _id_ville(champ_ville):
        """Identifiant Météociel associé au champ 'ville' fourni, si connu
        (posé par _choisir_ville, ou par _proposer_choix_villes après
        désambiguïsation) - None si l'utilisateur a tapé/modifié le texte
        manuellement depuis (voir le connect sur textEdited, qui invalide
        systématiquement cette propriété), auquel cas Météociel retombe sur
        sa recherche classique par nom."""
        valeur = champ_ville.property("meteociel_id")
        return valeur if valeur else None

    def _proposer_choix_villes(self, candidats, champ_ville, relancer, sur_annulation=None):
        ...
        choisit une, met à jour ``champ_ville`` (avec son ``meteociel_id``,
        ce qui élimine toute ambiguïté pour le reste de la session - voir
        _id_ville) puis appelle ``relancer`` pour relancer automatiquement
        la récupération météo avec le nom exact choisi.

        Générique : utilisée par les trois onglets météo (Décision,
        Graphiques, Historique), qui peuvent chacun recevoir cette
        exception (voir meteo_decision._resoudre_et_appeler_previsions /
        recuperer_historique) dès qu'une ville est tapée à la main - sans
        passer par le bouton 📍 - et correspond à plusieurs communes
        françaises (ex. les deux communes nommées "Buc")."""
        etiquettes = [
        ...
        champ_ville.setText(candidats[index]["nom"])
        champ_ville.setProperty("meteociel_id", candidats[index].get("id"))
        relancer()
    ...
    def _lancer_chargement_historique(self):
        ...
        ville_id = self._id_ville(self.edit_hist_ville)
        if not ville:
        ...
        thread = WorkerThread(md.recuperer_historique, ville, d1, d2, ville_id=ville_id)
    ...
    def _historique_termine(self, ville_trouvee, df, erreur):
        ...
        if isinstance(erreur, md.PlusieursVillesTrouvees):
            def annuler():
                ...
                self.lbl_hist_status.setStyleSheet(f"color: {COULEUR_ALERTE};")
            self._proposer_choix_villes(
                erreur.candidats, self.edit_hist_ville,
                self._lancer_chargement_historique, annuler)
            return
        if erreur is not None:
