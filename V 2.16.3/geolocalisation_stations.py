...
# Priorité utilisée pour choisir quelle station garder quand plusieurs
# stations distinctes produisent EXACTEMENT le même libellé affiché une
# fois mis en forme (voir _dedupliquer_par_etiquette ci-dessous) - ex. deux
# stations du même village, l'une "secondaire" l'autre "amateur", dont le
# nom "embelli" est devenu identique (alors qu'il ne l'était pas dans la
# base brute d'origine). On privilégie les stations officielles
# ("secondaire", puis les réseaux synop/metar) avant les stations
# "amateur", elles-mêmes avant les stations "inactive".
...
def _dedupliquer_par_etiquette(entrees):
    """Ne garde qu'une entrée par libellé affiché (voir etiquette_ville)
    parmi ``entrees`` (liste de dicts ayant au moins "nom", "code_postal"
    et "type") : plusieurs stations Météociel distinctes partagent parfois
    exactement le même nom (une fois "embelli" pour l'affichage) et le même
    code postal - sans cette déduplication, elles apparaissent comme des
    doublons dans les résultats de recherche (pop-up "Choisir une
    localisation", utilisée aussi bien depuis l'onglet Historique que
    Décision/Graphique). En cas de collision, on garde la station la plus
    fiable (voir _PRIORITE_TYPE_STATION) plutôt qu'une des deux au hasard.

    Préserve l'ordre relatif des entrées conservées (utile quand
    ``entrees`` est déjà triée, ex. par distance croissante)."""
    meilleures = {}
    for e in entrees:
        cle = etiquette_ville(e["nom"], e.get("code_postal"), e.get("type"))
        existante = meilleures.get(cle)
        if existante is None or _priorite_type_station(e.get("type")) < _priorite_type_station(existante.get("type")):
            meilleures[cle] = e
...
def _nom_affichage(names):
    ...
    if "Arrondissement" == name[-14:]:
        name = " ".join([(i.lower() if i[0].isdigit() else i) for i in name.split(" ")])
    return name
...
def charger_stations(force=False):
    ...
    Renvoie une liste de dicts {"id", "nom", "type", "lat", "lon",
    "code_postal"} (``code_postal`` vaut None si inconnu - voir
    _charger_codes_postaux) - liste vide si les fichiers de données sont
    absents."""
    ...
    stations = _dedupliquer_par_etiquette(stations)

    _cache["stations"] = stations
    return stations


def etiquette_ville(nom, code_postal, type_):
    """Libellé d'affichage standard pour une ville/station - IDENTIQUE quel
    que soit l'onglet de sélection (recherche par nom, carte interactive,
    coordonnées GPS - voir dialogue_localisation.py), pour que la valeur
    finalement renvoyée à l'application ne dépende pas de l'onglet utilisé
    pour la trouver : "Nom (code_postal)" quand le code postal est connu,
    sinon "Nom  —  (type)"."""
    if code_postal:
        return f"{nom} ({code_postal})"
    return f"{nom}  —  ({type_})"
...
def rechercher_villes(motif, pays="france", limite=200):
    ...
    resultats = _dedupliquer_par_etiquette(resultats)
