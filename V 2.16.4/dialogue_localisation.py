...
class DialogueChoixLocalisation(QDialog):
    ...
    def _recherche_nom_changee(self, texte):
        ...
        if not candidats:
            ...
            return
        for c in candidats:
            texte_item = gs.etiquette_ville(c["nom"], c["code_postal"], c["type"])
            item = QListWidgetItem(texte_item)
            item.setData(Qt.UserRole, {"id": c["id"], "nom": texte_item})
            self.liste_resultats.addItem(item)
    ...
    def _afficher_stations_proches(self, lat, lon):
        ...
        if not proches:
            ...
            return
        for s in proches:
            etiquette = gs.etiquette_ville(s["nom"], s.get("code_postal"), s["type"])
            texte = f"{etiquette}  —  {s['distance_km']:.1f} km"
            item = QListWidgetItem(texte)
            # La distance n'est qu'une information d'affichage propre à cet
            # onglet : la valeur renvoyée à l'application (UserRole) est
            # l'id + l'étiquette seule (nom + code postal), rigoureusement
            # identique au format produit par l'onglet "Recherche par nom".
            item.setData(Qt.UserRole, {"id": s["id"], "nom": etiquette})
            self.liste_resultats.addItem(item)
        self.liste_resultats.setCurrentRow(0)
    ...
    def station_choisie(self):
        """Dict {"id", "nom"} de la station sélectionnée dans la liste de
        résultats, ou None si aucune sélection valide. ``id`` est
        l'identifiant Météociel de la station (city_id), ``nom`` son
        libellé affiché (nom + code postal, ou nom + type)."""
        items = self.liste_resultats.selectedItems()
    ...
    def nom_station_choisie(self):
        """Nom (libellé affiché) de la station sélectionnée dans la liste
        de résultats, ou None si aucune sélection valide. Conservé pour les
        appelants qui n'ont besoin que du libellé (ex. onglets Décision et
        Graphique) - voir station_choisie() pour récupérer aussi l'id."""
        station = self.station_choisie()
        return station["nom"] if station else None
...
def choisir_localisation_avec_id(parent=None):
    """Ouvre la pop-up de choix de localisation et renvoie un tuple
    (city_id, nom) pour la station choisie, ou (None, None) si
    l'utilisateur a annulé / n'a rien sélectionné.

    À utiliser à la place de ``choisir_localisation`` quand l'appelant a
    besoin de l'identifiant Météociel de la station (ex. historique météo -
    voir meteo_decision.recuperer_historique_par_id), afin d'éviter d'avoir
    à ré-identifier la ville à partir de son seul nom affiché."""
    dialogue = DialogueChoixLocalisation(parent)
    if dialogue.exec_() == QDialog.Accepted:
        station = dialogue.station_choisie()
        if station:
            return station["id"], station["nom"]
    return None, None
