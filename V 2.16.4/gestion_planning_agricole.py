...
class PlanningApp(QMainWindow):
    ...
    def _build_onglet_historique(self, parent):
        ...
        # La station effectivement choisie via la pop-up de localisation
        # (city_id Météociel) est mémorisée à part - voir
        # _hist_ville_choisie et _hist_ville_modifiee_manuellement - plutôt
        # que d'être ré-identifiée à partir du seul texte affiché, qui peut
        # être ambigu (plusieurs villes françaises homonymes) ou correspondre
        # à plusieurs stations distinctes partageant le même libellé.
        self.hist_city_id = None
        self.edit_hist_ville.textEdited.connect(self._hist_ville_modifiee_manuellement)
        form_layout.addWidget(self.edit_hist_ville)
        btn_hist_localiser = QPushButton("📍")
        btn_hist_localiser.setToolTip("Choisir sur une carte, par coordonnées GPS ou par nom")
        btn_hist_localiser.setMaximumWidth(36)
        btn_hist_localiser.clicked.connect(self._choisir_ville_historique)
    ...
    # ------------------------------------------------------------------
    # Sélection de la ville (avec identifiant Météociel résolu)
    # ------------------------------------------------------------------
    def _choisir_ville(self, champ_ville):
        """Ouvre la pop-up « Choisir une localisation » (carte cliquable,
        coordonnées GPS ou recherche par nom) et, si l'utilisateur valide un
        choix, met à jour le champ de saisie 'ville' Météociel fourni.
        Utilisé par les onglets Décision et Graphique, qui n'ont besoin que
        du libellé affiché - voir _choisir_ville_historique pour l'onglet
        Historique, qui mémorise en plus le city_id résolu."""
        nom = dl.choisir_localisation(self)
        if nom:
            champ_ville.setText(nom)

    def _choisir_ville_historique(self):
        """Ouvre la pop-up « Choisir une localisation » et mémorise à la
        fois le libellé affiché et l'identifiant Météociel (city_id) de la
        station choisie - voir dialogue_localisation.choisir_localisation_avec_id.
        Contrairement aux onglets Décision/Graphique (qui n'ont besoin que
        du libellé), l'Historique réutilise cet identifiant pour éviter
        toute ré-identification par nom lors du chargement (voir
        _lancer_chargement_historique / meteo_decision.recuperer_historique_par_id)."""
        city_id, nom = dl.choisir_localisation_avec_id(self)
        if nom:
            self.edit_hist_ville.setText(nom)
            self.hist_city_id = city_id

    def _hist_ville_modifiee_manuellement(self, _texte):
        """Le champ 'ville' a été modifié à la main (et non via la pop-up
        de localisation) : l'identifiant précédemment mémorisé ne
        correspond plus forcément au texte affiché, on l'invalide - le
        chargement se repliera alors sur une résolution par nom (voir
        _lancer_chargement_historique)."""
        self.hist_city_id = None

    def _proposer_choix_villes(self, candidats):
        """Affiche un choix parmi plusieurs villes françaises homonymes (voir
        meteo_decision.PlusieursVillesTrouvees - ne se produit que dans le
        cas replié où aucun city_id n'était mémorisé, ex. saisie manuelle)
        et, si l'utilisateur en choisit une, mémorise son identifiant et son
        nom puis relance automatiquement le chargement."""
        etiquettes = [
        ...
        # setText déclenche textChanged mais pas textEdited (réservé aux
        # modifications faites par l'utilisateur au clavier) : hist_city_id
        # n'est donc pas ré-invalidé par _hist_ville_modifiee_manuellement ici.
        self.edit_hist_ville.setText(candidat["nom"])
        self.hist_city_id = candidat["id"]
    ...
    def _lancer_chargement_historique(self):
        ...
        # Priorité au city_id déjà résolu (station choisie via la pop-up de
        # localisation, sans ambiguïté possible) ; à défaut (champ modifié
        # à la main), on retombe sur l'ancienne résolution par nom, qui peut
        # lever PlusieursVillesTrouvees - voir _historique_termine.
        if self.hist_city_id:
            thread = WorkerThread(md.recuperer_historique_par_id, self.hist_city_id, d1, d2)
        else:
    ...
    def _historique_termine(self, ville_trouvee, df, erreur):
        ...
        if isinstance(erreur, md.PlusieursVillesTrouvees):
            ...
            return
        if isinstance(erreur, ValueError) and self.hist_city_id:
            # city_id mémorisé mais introuvable dans cities_database_old.json
            # (fichier absent, id périmé...) : on se replie une seule fois
            # sur l'ancienne résolution par nom plutôt que d'échouer.
            self.hist_city_id = None
            self.lbl_hist_status.setText(
                "⚠ Identifiant de station introuvable localement, nouvelle tentative par nom..."
            )
            self._lancer_chargement_historique()
            return
        if erreur is not None:
