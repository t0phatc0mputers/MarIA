...
# Dépendances de la carte interactive (Leaflet / geopandas / folium /
# mapclassify) : optionnelles, importées à part pour que le reste de
# l'application (carte schématique et carte "contours réels" en
# QGraphicsView) continue de fonctionner même si ces bibliothèques ne sont
# pas installées.
try:
    import geopandas as gpd
    import pandas as pd
    import folium
    import mapclassify  # noqa: F401  (utilisé implicitement par folium.Choropleth "scheme=")
    CARTE_WEB_DISPONIBLE = True
except ImportError:
    CARTE_WEB_DISPONIBLE = False

DOSSIER_RESSOURCES_CARTE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ressources_carte")
GEOJSON_REGIONS = os.path.join(DOSSIER_RESSOURCES_CARTE, "regions_france.geojson")
...
# Température moyenne annuelle indicative (°C), simple valeur numérique
# représentative par région (moyenne approximative, usage strictement
# cartographique/pédagogique) : sert de base numérique à la classification
# automatique (mapclassify) de la couche "climat".
TEMP_MOY_INDICATIF_C = {
    "Hauts-de-France": 10.5, "Normandie": 11.0, "Île-de-France": 11.5,
    "Grand Est": 10.0, "Bretagne": 11.5, "Pays de la Loire": 12.0,
    "Centre-Val de Loire": 11.5, "Bourgogne-Franche-Comté": 10.5,
    "Nouvelle-Aquitaine": 13.0, "Auvergne-Rhône-Alpes": 11.0,
    "Occitanie": 13.5, "Provence-Alpes-Côte d'Azur": 14.5, "Corse": 15.0,
}
for _nom, _d in REGIONS.items():
    _d["temp_moy_indicatif_c"] = TEMP_MOY_INDICATIF_C[_nom]
...
# ------------------------------------------------------------------
# Carte interactive Leaflet (geopandas + folium + mapclassify)
# ------------------------------------------------------------------
# Contrairement aux deux cartes ci-dessus (dessinées "à la main" avec
# QGraphicsView, pour un rendu garanti sans dépendance lourde), ce mode
# s'appuie sur une vraie pile SIG :
#   - geopandas charge le contour réel des régions (WGS84, fichier
#     ressources_carte/regions_france.geojson) et fusionne les attributs
#     climat/sol du dict REGIONS ci-dessus dans un GeoDataFrame ;
#   - mapclassify calcule les seuils de classes (par défaut : "NaturalBreaks",
#     l'algorithme de Jenks) à partir des valeurs numériques de la région
#     (température moyenne indicative pour le climat, indice de fertilité
#     pour le sol) ;
#   - folium dessine la carte Leaflet interactive (zoom/pan tactile à la
#     souris, tuiles de fond, choroplèthe colorée selon les classes
#     calculées, info-bulles) et exporte le résultat en HTML autonome.
_GDF_CACHE = None


def _construire_geodataframe():
    """Construit (une seule fois, mise en cache) le GeoDataFrame des 13
    régions avec leurs attributs climat/sol. Lève RuntimeError si
    geopandas/pandas ne sont pas installés."""
    global _GDF_CACHE
    if not CARTE_WEB_DISPONIBLE:
        raise RuntimeError("geopandas / folium / mapclassify ne sont pas installés.")
    if _GDF_CACHE is not None:
        return _GDF_CACHE

    gdf = gpd.read_file(GEOJSON_REGIONS)
    lignes = [{"nom": nom, **d} for nom, d in REGIONS.items()]
    attrs = pd.DataFrame(lignes)[
        ["nom", "climat", "climat_desc", "temp_moy_indicatif_c",
         "sol_type", "sol_niveau", "sol_desc"]
    ]
    gdf = gdf.merge(attrs, on="nom", how="left")
...
def construire_carte_html(mode_index, scheme="NaturalBreaks", k=5):
    """Construit la carte Leaflet interactive (folium) pour le mode demandé
    (0 = climat, 1 = qualité du sol) et renvoie le HTML autonome (str) prêt
    à être affiché dans un QWebEngineView ou écrit dans un fichier.

    La classification des classes de la choroplèthe est calculée par
    mapclassify via le paramètre ``scheme`` de folium.Choropleth (schéma de
    Jenks/"NaturalBreaks" par défaut, adapté à des distributions
    hétérogènes comme ici)."""
    gdf = _construire_geodataframe()

    if mode_index == 0:
        colonne, echelle, legende = "temp_moy_indicatif_c", "YlOrRd", \
            "Température moyenne annuelle indicative (°C)"
        champs_infobulle = ["nom", "climat", "temp_moy_indicatif_c"]
        alias_infobulle = ["Région", "Climat dominant", "Temp. moy. indicative (°C)"]
    else:
        colonne, echelle, legende = "sol_niveau", "RdYlGn", \
            "Qualité indicative du sol (1 = faible, 5 = très favorable)"
        champs_infobulle = ["nom", "sol_type", "sol_niveau"]
        alias_infobulle = ["Région", "Type de sol dominant", "Indice de fertilité"]

    carte = folium.Map(location=[46.6, 2.5], zoom_start=6, tiles="CartoDB positron",
                        control_scale=True)

    nb_valeurs_uniques = gdf[colonne].nunique()
    k_effectif = min(k, nb_valeurs_uniques)
    try:
        folium.Choropleth(
            geo_data=gdf,
            data=gdf,
        ...
        # Repli sans mapclassify (ex. distribution dégénérée) : choroplèthe
        # continue simple, sans schéma de classification.
        folium.Choropleth(
            geo_data=gdf, data=gdf, columns=["nom", colonne],
            key_on="feature.properties.nom", fill_color=echelle,
            fill_opacity=0.75, line_opacity=0.4, legend_name=legende,
        ).add_to(carte)

    folium.GeoJson(
        gdf,
        style_function=lambda f: {"fillOpacity": 0, "color": "#333333", "weight": 0.7},
        highlight_function=lambda f: {"weight": 2.5, "color": "#0b5a9e"},
        tooltip=folium.GeoJsonTooltip(fields=champs_infobulle, aliases=alias_infobulle,
                                       sticky=True),
        popup=folium.GeoJsonPopup(
            fields=["nom", "climat_desc"] if mode_index == 0 else ["nom", "sol_desc"],
            aliases=["Région", "Détail"] if mode_index == 0 else ["Région", "Détail"],
            max_width=320,
        ),
    ).add_to(carte)
...
def enregistrer_carte_html(mode_index, chemin, scheme="NaturalBreaks", k=5):
    """Génère la carte (cf. construire_carte_html) et l'écrit dans
    ``chemin`` ; renvoie ce même chemin."""
    html = construire_carte_html(mode_index, scheme=scheme, k=k)
