...
# Palette pour une interface plus lisible / intuitive
COULEUR_OK = "#2e7d32"
COULEUR_ALERTE = "#c62828"
COULEUR_INFO = "#555555"
COULEUR_ACCENT = "#0b5a9e"
COULEUR_ACCENT_CLAIR = "#e7f1fb"
COULEUR_ACCENT_HOVER = "#0e6fc2"
COULEUR_FOND = "#f4f7fa"
COULEUR_CARTE = "#ffffff"
COULEUR_BORDURE = "#d5dde5"
COULEUR_ALT_LIGNE = "#eef4fa"
...
QTabBar::tab:hover:!selected {{
    background-color: {COULEUR_ACCENT_CLAIR};
}}
...
QLineEdit, QPlainTextEdit, QComboBox, QSpinBox, QDateEdit {{
    ...
    border-radius: 5px;
    padding: 4px 6px;
    selection-background-color: {COULEUR_ACCENT};
}}
...
QHeaderView::section {{
    ...
    padding: 6px;
    border: none;
    border-right: 1px solid #0a4f88;
    font-weight: 600;
}}

QTableWidget {{
    background-color: {COULEUR_CARTE};
    alternate-background-color: {COULEUR_ALT_LIGNE};
...
class PlanningTableWidget(QTableWidget):
    """QTableWidget avec réordonnancement des lignes par glisser-déposer.

    Qt ne gère pas nativement le déplacement de lignes pour un QTableWidget
    (contrairement à QListWidget) : on intercepte donc l'événement de dépôt
    pour calculer la ligne source et la ligne cible, puis on délègue le
    déplacement réel des données à l'application via le signal
    ``rowsReordered`` (l'application reconstruit ensuite le tableau à partir
    de son modèle de données, ce qui reste correct même si un filtre ou une
    recherche est actif).
    """
    ...
    def dropEvent(self, event):
        if event.source() is not self:
            event.ignore()
            return
        ligne_source = self.currentRow()
        index_cible = self.indexAt(event.pos())
        ligne_cible = index_cible.row()
        if ligne_cible == -1:
            ligne_cible = self.rowCount() - 1
        if ligne_source != -1 and ligne_cible != -1 and ligne_source != ligne_cible:
            self.rowsReordered.emit(ligne_source, ligne_cible)
        event.accept()
...
class PlanningApp(QMainWindow):
    ...
    def _build_onglet_planning(self, parent):
        ...
        astuce = QLabel("💡 Astuce : glissez-déposez une ligne du tableau pour la réordonner "
                         "(fonctionne aussi avec un filtre actif).")
        astuce.setStyleSheet(f"color: {COULEUR_INFO}; font-style: italic;")
        ...
        self.table = PlanningTableWidget(0, len(FIELDS))
        ...
        # Glisser-déposer : on peut réordonner les lignes du planning en les
        # faisant glisser à une autre position dans le tableau.
        self.table.setDragEnabled(True)
        self.table.setAcceptDrops(True)
        self.table.setDropIndicatorShown(True)
        self.table.setDragDropMode(QAbstractItemView.InternalMove)
        self.table.setDefaultDropAction(Qt.MoveAction)
        self.table.rowsReordered.connect(self._on_ligne_deplacee)
    ...
    def _on_ligne_deplacee(self, ligne_source_pos, ligne_cible_pos):
        """Applique un déplacement de ligne (glisser-déposer) au modèle de
        données réel (self.rows), en tenant compte d'un éventuel filtre
        actif : on travaille via les index d'origine mémorisés dans
        self._visible plutôt que via la position visuelle brute."""
        if not (0 <= ligne_source_pos < len(self._visible)) or not (0 <= ligne_cible_pos < len(self._visible)):
            return
        idx_source = self._visible[ligne_source_pos][0]
        idx_cible = self._visible[ligne_cible_pos][0]
        if idx_source == idx_cible:
            return

        entree = self.rows.pop(idx_source)
        if idx_source < idx_cible:
            idx_cible -= 1
        self.rows.insert(idx_cible, entree)

        self._rafraichir_tableau()
        self._set_status(f"Entrée « {entree.get('culture', '')} » déplacée.")

        # Resélectionne la ligne déplacée pour un retour visuel immédiat.
        for row_pos, (idx, _r) in enumerate(self._visible):
            if idx == idx_cible:
    ...
    def _lancer_chargement_historique(self):
        ...
        thread.succes.connect(
            lambda resultat: self._historique_termine(resultat[0], resultat[1], resultat[2], None))
        thread.echec.connect(lambda erreur: self._historique_termine(None, None, [], erreur))
        thread.finished.connect(lambda: self._nettoyer_thread(thread))
        self._threads.append(thread)
        thread.start()

    def _historique_termine(self, ville_trouvee, df, jours_echoues, erreur):
        ...
        if erreur is not None:
            ...
            return
        if df is None or df.empty:
            texte = "Aucune donnée renvoyée par Météociel pour cette période."
            if jours_echoues:
                texte += f" ({len(jours_echoues)} jour(s) n'ont pas pu être récupérés.)"
            self.lbl_hist_status.setText(texte)
            self.lbl_hist_status.setStyleSheet(f"color: {COULEUR_ALERTE};")
            return
        ...
        # On retire les éventuelles valeurs manquantes plutôt que de laisser
        # matplotlib tracer une ligne invisible / vide si une colonne
        # contient uniquement des NaN pour la période demandée.
        df_temp = df.dropna(subset=["temperature"])
        df_humid = df.dropna(subset=["humidity"])

        if df_temp.empty:
            self.ax_hist_temp.text(0.5, 0.5, "Pas de valeur de température exploitable",
                                    ha="center", va="center", transform=self.ax_hist_temp.transAxes,
                                    color=COULEUR_ALERTE)
        else:
            self.ax_hist_temp.plot(df_temp["datetime"], df_temp["temperature"],
                                    color="#d9534f", linewidth=1.4, marker="o", markersize=3)
        ...
        if df_humid.empty:
            self.ax_hist_humid.text(0.5, 0.5, "Pas de valeur d'humidité exploitable",
                                     ha="center", va="center", transform=self.ax_hist_humid.transAxes,
                                     color=COULEUR_ALERTE)
        else:
            self.ax_hist_humid.plot(df_humid["datetime"], df_humid["humidity"],
                                     color="#5cb85c", linewidth=1.4, marker="o", markersize=3)
        ...
        texte_statut = f"✅ Historique chargé pour « {ville_trouvee} » ({len(df)} relevés)."
        if jours_echoues:
            noms_jours = ", ".join(j.strftime("%d/%m") for j in jours_echoues)
            texte_statut += f" ⚠ {len(jours_echoues)} jour(s) ignoré(s) (indisponibles) : {noms_jours}."
        self.lbl_hist_status.setText(texte_statut)
        self.lbl_hist_status.setStyleSheet(f"color: {COULEUR_OK if not jours_echoues else '#8a6d00'};")
