...
def recuperer_historique(ville: str, date_debut: datetime.date, date_fin: datetime.date):
    ...
    Comme un appel réseau distinct est effectué par jour, une erreur
    ponctuelle sur un seul jour (timeout, erreur temporaire du site, etc.)
    n'interrompt plus toute la récupération : ce jour est simplement ignoré
    et l'agrégation se poursuit sur les jours restants (c'est la cause la
    plus fréquente d'un graphique d'historique qui reste vide : sur une
    période de plusieurs jours, il suffisait qu'UN SEUL jour échoue pour que
    toute la requête échoue et qu'aucune donnée ne soit affichée). Si TOUS
    les jours échouent, l'exception rencontrée sur le premier jour est
    relevée. La liste des jours ignorés est retournée en 3e élément du
    tuple.
    """
    if not METEOCIEL_AVAILABLE:
        raise RuntimeError(
    ...
    for jour in jours:
        try:
            ...
            # Un jour en échec (timeout, erreur temporaire du site, etc.) ne
            # doit pas faire échouer toute la période : on le note et on
            # continue avec les jours suivants.
            jours_echoues.append(jour)
        ...
        # Sécurité supplémentaire : force les colonnes numériques attendues
        # pour le tracé (température, humidité...) à un type numérique. Si
        # meteociel-api renvoie une valeur inattendue (texte, cellule vide),
        # cela évite d'obtenir une colonne de type "object" que matplotlib
        # refuserait silencieusement de tracer plutôt que de faire échouer
        # tout le chargement.
        for col in ("temperature", "humidity"):
            if col in df_jour.columns:
                df_jour[col] = pd.to_numeric(df_jour[col], errors="coerce")
    ...
    if not trames:
        ...
        return ville_trouvee, pd.DataFrame(), jours_echoues
    ...
    return ville_trouvee, combined, jours_echoues
