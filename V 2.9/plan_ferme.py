...
def serialiser(rects):
    """rects : liste de dicts {id, parent, nom, type, etat, culture,
    x, y, largeur, hauteur}. Retourne le texte du fichier."""
    lignes = ["PLAN_FERME 1", ""]
...
def sauvegarder_plan(path, scene):
    rects = []
    ...
    with open(path, "w", encoding="utf-8") as f:
        f.write(serialiser(rects))


def charger_plan(path, scene):
    """Vide la scène et recharge le plan depuis le fichier texte. Retourne
    le prochain id disponible (à utiliser pour les rectangles créés
    ensuite)."""
    with open(path, "r", encoding="utf-8") as f:
        rects = deserialiser(f.read())
