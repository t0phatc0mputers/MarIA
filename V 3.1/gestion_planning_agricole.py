...
from PyQt5.QtWidgets import (
    ...
    QTextBrowser, QStackedWidget, QInputDialog, QSplashScreen,
)

# Les modules métier et bibliothèques ci-dessous (météo, cartographie,
# matplotlib, moteur web...) sont volontairement chargés plus tard, une fois
# la fenêtre de chargement déjà affichée à l'écran (voir
# _importer_modules_lourds), afin que cette dernière apparaisse
# immédiatement au lancement du programme plutôt qu'après ces imports, qui
# peuvent prendre plusieurs secondes. Les noms sont déclarés ici à None/False
# pour que le reste du fichier puisse s'y référer sans erreur avant que
# l'import effectif n'ait eu lieu.
...
def _importer_modules_lourds(splash=None):
    ...
    matplotlib, moteur web Chromium pour la carte...). Appelée après
    l'affichage de la fenêtre de chargement, en mettant à jour son message,
    pour que l'utilisateur voie tout de suite un retour visuel au lancement
    plutôt qu'un écran figé pendant ces imports."""
    ...
    _maj("Chargement de la carte interactive...")
    try:
...
class PlanningApp(QMainWindow):
    ...
    def _build_onglet_accueil(self, parent):
        ...
        cartes = [
            (1, "📋 Planning cultural", "Le cœur de l'application",
             "Commencez ici pour voir ou saisir vos cultures."),
            (2, "🌦 Aide à la décision", "Semis, plantation, récolte : le bon moment ?",
             "Entrez votre ville pour un conseil immédiat."),
            (3, "📈 Graphiques météo", "Courbes de température, pluie, vent",
             "Utile pour anticiper sur plusieurs jours."),
            (4, "🔄 Rotation des cultures", "Éviter les maladies liées au sol",
             "Vérifiez avant de replanter une parcelle."),
            (5, "🧪 Analyse de sol & Fertilisation", "Comprendre une analyse et calculer les apports",
             "Pratique si vous avez un bulletin d'analyse."),
            (6, "🌱 Fiches de référence", "Une fiche pratique par culture",
             "Idéal pour un débutant : tout y est expliqué."),
            (7, "🚜 Plan de la ferme", "Décrire vos parcelles",
             "À faire une première fois, puis à ajuster."),
            (8, "🗺️ Carte régionale", "Climat et sol de votre région",
