...
# ------------------------------------------------------------------
# Score "rotation" : interdiction (retour trop rapide de la même famille
# botanique) + bonus/malus recommandé/possible, calculés sur l'ENSEMBLE de
# l'historique cultural de la planche - pas seulement sa toute dernière
# culture (voir plan_ferme.DialogHistoriqueParcelle pour la saisie de cet
# historique).
# ------------------------------------------------------------------
_RE_ANNEE = re.compile(r"(\d{4})")


def _annee_depuis_texte(texte):
    """Extrait une année à 4 chiffres depuis un texte libre de période
    (ex. "2022" -> 2022, "hiver 2022-2023" -> 2022, "15/03/2022" -> 2022),
    ou None si aucune année n'est reconnaissable."""
    if not texte:
        return None
    m = _RE_ANNEE.search(str(texte))
    return int(m.group(1)) if m else None


def _annee_fin_periode(periode):
    """periode : tuple (debut, fin) en texte libre. Renvoie l'année de fin
    si reconnaissable, sinon celle de début, sinon None."""
    debut, fin = periode
    return _annee_depuis_texte(fin) or _annee_depuis_texte(debut)


def _entree_la_plus_recente(historique):
    """Renvoie l'entrée ((debut, fin), culture) la plus récente d'un
    historique non vide (celle dont la fin a l'année la plus élevée), ou
    la dernière entrée de la liste si aucune année n'y est exploitable."""
    plus_recente = historique[-1]
    meilleure_annee = None
    for entree in historique:
        annee = _annee_fin_periode(entree[0])
        if annee is not None and (meilleure_annee is None or annee > meilleure_annee):
            meilleure_annee, plus_recente = annee, entree
    return plus_recente


def score_rotation(culture_nom, historique, fb, rc, annee_reference=None):
    """historique : liste de ((debut, fin), culture) - le passé cultural
    complet de la planche, pas seulement la dernière culture connue.
    `annee_reference` : année civile de la prochaine plantation envisagée,
    utilisée pour calculer le nombre d'années écoulées depuis chaque
    culture passée. Renvoie (score, motif). score=0 signifie "interdit".

    Amélioration par rapport à l'ancienne version (qui ne regardait que la
    toute dernière culture) : CHAQUE entrée de l'historique est comparée
    séparément à son propre délai de retour conseillé. Une famille
    cultivée il y a 2 ans reste ainsi détectée comme interdite si son
    délai de retour est de 4 ans, même si la planche a porté une culture
    d'une autre famille l'année suivante - un cas que l'ancienne version,
    qui ne connaissait que la toute dernière culture, ne pouvait pas
    repérer.

    Si aucune année n'est exploitable pour une entrée donnée (période
    saisie sans année, ou `annee_reference` absente), on retombe sur la
    règle impérative d'origine pour cette entrée-là (interdiction sans
    condition de délai) par prudence."""
    if not historique:
        return 100, ""

    for periode, culture_passee in historique:
        if not culture_passee or culture_passee not in fb.FICHES:
            continue
        analyse = rc.analyser_rotation(culture_passee)
        if analyse is None:
            continue
        noms_a_eviter = {e["nom"] for e in analyse["a_eviter"]}
        if culture_nom not in noms_a_eviter:
            continue
        delai = analyse["delai_retour_annees"]
        annee_passee = _annee_fin_periode(periode)
        if annee_reference is None or annee_passee is None:
            return 0, f"même famille que « {culture_passee} » (délai conseillé : {delai} ans)"
        ecart = annee_reference - annee_passee
        if ecart < delai:
            return 0, (f"même famille que « {culture_passee} » cultivée il y a "
                       f"{ecart} an(s) (délai conseillé : {delai} ans)")
        # Délai déjà écoulé pour cette entrée : pas d'interdiction, mais on
        # continue à examiner le reste de l'historique.

    # Aucune interdiction : le bonus/malus "recommandé / possible" se base
    # sur la culture la plus récente, comme dans l'ancienne version.
    _, derniere_culture = _entree_la_plus_recente(historique)
...
# ------------------------------------------------------------------
def lister_cultures_triees(nom_region, semaine_ref, historique, annee_reference,
    ...
    triée par score décroissant. `historique` (liste de ((debut, fin),
    culture)) peut être vide (mode "je commence de zéro" : pas de
    contrainte de rotation). `annee_reference` : année civile de la
    prochaine plantation, utilisée pour calculer le délai de retour propre
    à chaque culture de l'historique (voir score_rotation)."""
    resultats = []
    for nom_culture, fiche in fb.FICHES.items():
        ...
        s_rotation, motif = score_rotation(nom_culture, historique, fb, rc, annee_reference)
...
def generer_disposition_defaut(nb_zones, nb_chapelles, nb_planches):
    ...
    historique} (derniere_culture et historique toujours vides dans ce
    cas)."""
    planches = []
...
def importer_planches_depuis_plan(chemin, pf):
    ...
    `historique` reprend le passé cultural complet saisi sur le rectangle
    (champ `historique=`), complété par sa culture actuellement en place
    (champ `culture=`, sans date connue) si elle est renseignée.
    `derniere_culture` (conservée pour l'affichage et la compatibilité)
    est la culture la plus récente de cet historique combiné."""
    ...
    for r in rects:
        if r["type"] != "Planche":
            continue
        historique = list(r.get("historique", []))
        if r.get("culture"):
            # Culture actuellement en place, sans date connue : traitée
            # comme "dernière culture" et soumise à la règle impérative
            # (voir score_rotation, cas annee_passee/annee_reference None).
            historique.append((("", ""), r["culture"]))
        derniere = _entree_la_plus_recente(historique)[1] if historique else ""
        planches.append({
...
class DialogueDispositionFerme(QDialog):
    ...
    def planches(self):
        ...
        for row in range(self.table.rowCount()):
            ...
            # Saisie manuelle : une seule culture connue, sans date - donc
            # sans année exploitable pour le calcul du délai de retour
            # (voir score_rotation, qui applique alors la règle impérative
            # par prudence, comme le faisait l'ancienne version).
            historique = [(("", ""), derniere)] if derniere else []
            resultats.append({"zone": zone, "chapelle": chapelle, "planche": planche,
                               "derniere_culture": derniere, "historique": historique})
        return resultats
...
##            "grisées violent la règle de rotation (même famille botanique que la dernière culture)."))
    ...
    def _historique_chaine(self, idx):
        """Historique complet à prendre en compte pour la rotation : le
        passé cultural connu de la planche (mode "j'ai déjà cultivé" ou
        import d'un plan de ferme, avec ses dates) complété par les étapes
        déjà ajoutées à la chaîne dans cette session (avec leur année,
        pour un calcul de délai précis dès la 2e étape planifiée)."""
        historique = list(self.planches[idx].get("historique", []))
        for etape in self.assignations.get(idx, []):
            historique.append(((str(etape["annee_debut"]), str(etape["annee_fin"])), etape["culture"]))
        return historique
    ...
    def _rafraichir_liste_candidats(self, idx):
        ...
        historique = self._historique_chaine(idx)
        cultures = lister_cultures_triees(
            self.nom_region, semaine, historique, annee,
