...
- action         : Travail du sol / Apport de compost / Apport de fumier /
                    Paillage / Semis direct / Semis en pot/plant / Plantation /
                    Récolte / Forçage
...
ACTIONS = ["Travail du sol", "Apport de compost", "Apport de fumier", "Paillage",
           "Semis direct", "Semis en pot/plant", "Plantation", "Récolte", "Forçage"]

# Nom de l'ancienne action "Conservation", désormais totalement retirée du
# planning (aussi bien de la liste ACTIONS ci-dessus que des données CSV
# chargées : voir _load_csv). Conservée ici uniquement pour filtrer les
# fichiers CSV existants qui contiendraient encore d'anciennes lignes.
ACTION_SUPPRIMEE_CONSERVATION = "Conservation"
...
# Regroupement agronomique des actions, utilisé pour organiser l'affichage
# du planning cultural : à l'intérieur d'une même culture, les actions sont
# triées selon cet enchaînement logique plutôt que dans un ordre quelconque.
GROUPES_ACTIONS = [
    ("Travail du sol", ["Travail du sol"]),
    ("Compost / Paillage", ["Apport de compost", "Apport de fumier", "Paillage"]),
    ("Semis (direct / pot)", ["Semis direct", "Semis en pot/plant"]),
    ("Plantation", ["Plantation"]),
    ("Récolte", ["Récolte"]),
    ("Forçage", ["Forçage"]),
]
# action -> position dans l'enchaînement ci-dessus (utilisé comme clé de tri)
ORDRE_ACTIONS = {action: position
                  for position, (_, actions) in enumerate(GROUPES_ACTIONS)
...
class LabelChampFiche(QLabel):
    """QLabel avec retour à la ligne (word wrap), destiné aux valeurs
    affichées dans les fiches de référence (QFormLayout).

    Problème corrigé : sur macOS, QFormLayout ne recalcule pas toujours
    correctement la hauteur d'une ligne contenant un QLabel en word-wrap
    après un redimensionnement (au premier affichage ou après un
    changement de fiche). Le texte sur deux lignes se retrouve alors
    superposé sur une seule ligne de hauteur, au lieu que la ligne du
    formulaire s'agrandisse pour l'accueillir. On force ici la hauteur
    minimale du label à suivre sa largeur réelle à chaque redimensionnement
    (via heightForWidth), ce qui règle l'affichage sur toutes les
    plateformes, y compris macOS.
    """
    ...
    def setText(self, texte):
        # Le changement de fiche (sélection d'une autre culture) modifie le
        # texte sans forcément déclencher de redimensionnement : il faut
        # donc aussi réajuster la hauteur ici, sinon une valeur passant de
        # une à deux lignes reste affichée sur une seule ligne de hauteur.
        super().setText(texte)
        self._ajuster_hauteur()

    def _ajuster_hauteur(self):
        largeur = self.width()
        hauteur = self.heightForWidth(largeur) if largeur > 0 else self.sizeHint().height()
        if hauteur > 0 and self.minimumHeight() != hauteur:
            self.setMinimumHeight(hauteur)
            # Force le QFormLayout englobant à recalculer la place réservée
            # à cette ligne dès que la hauteur minimale change.
...
class PlanningApp(QMainWindow):
    ...
    def _lancer_didacticiel(self, relance_manuelle=False):
        ...
        etapes = [
            ...
            (2, "📅 Semaine",
             "Cette page résume, semaine par semaine, ce qu'il y a à faire d'après votre "
             "planning (semis, plantation, récolte, travail du sol...), et vous dit si la "
             "météo est favorable à chaque action. Un résumé de la semaine en cours est "
             "aussi visible directement sur l'onglet Accueil."),
            (3, "📋 Planning cultural",
    ...
    def _build_central(self):
        ...
        self._onglets_info = [
            ...
            (onglet_semaine, "📅 Semaine",
             "Quoi semer, planter, récolter et travailler cette semaine, selon le planning et la météo."),
        ...
        self._maj_splash("Chargement : Semaine...")
        self._build_onglet_semaine(onglet_semaine)
    ...
    def _build_onglet_accueil(self, parent):
        ...
        # --- Résumé "Cette semaine" (infos de l'onglet Semaine), visible
        # immédiatement sur la page d'accueil sans avoir à changer d'onglet ---
        self.box_accueil_semaine = QGroupBox()
        layout_accueil_semaine = QVBoxLayout(self.box_accueil_semaine)
        colonnes_semaine = ["Culture", "Conduite", "Action", "Semaines", "Commentaire"]
        self.table_accueil_semaine = QTableWidget(0, len(colonnes_semaine))
        self.table_accueil_semaine.setHorizontalHeaderLabels(colonnes_semaine)
        self.table_accueil_semaine.setEditTriggers(QAbstractItemView.NoEditTriggers)
        self.table_accueil_semaine.setSelectionBehavior(QAbstractItemView.SelectRows)
        self.table_accueil_semaine.setAlternatingRowColors(True)
        self.table_accueil_semaine.verticalHeader().setVisible(False)
        self.table_accueil_semaine.horizontalHeader().setStretchLastSection(True)
        self.table_accueil_semaine.setMaximumHeight(160)
        layout_accueil_semaine.addWidget(self.table_accueil_semaine)

        btn_voir_semaine = QPushButton("📅 Voir le détail de la semaine (avec la météo)")
        btn_voir_semaine.setCursor(Qt.PointingHandCursor)
        btn_voir_semaine.clicked.connect(lambda: self.tabs.setCurrentIndex(2))
        layout_accueil_semaine.addWidget(btn_voir_semaine)

        layout.addWidget(self.box_accueil_semaine)
        ...
        cartes = [
            ...
            (2, "📅 Semaine", "Quoi faire cette semaine, avec la météo",
             "Le résumé ci-dessus, complet, avec l'analyse météo de chaque action."),
            (3, "📋 Planning cultural", "Le cœur de l'application",
             "Commencez ici pour voir ou saisir vos cultures."),
            (4, "🌦 Aide à la décision", "Semis, plantation, récolte : le bon moment ?",
             "Entrez votre ville pour un conseil immédiat."),
            (5, "📈 Graphiques météo", "Courbes de température, pluie, vent",
             "Utile pour anticiper sur plusieurs jours."),
            (6, "🔄 Rotation des cultures", "Éviter les maladies liées au sol",
             "Vérifiez avant de replanter une parcelle."),
            (7, "🧪 Analyse de sol & Fertilisation", "Comprendre une analyse et calculer les apports",
             "Pratique si vous avez un bulletin d'analyse."),
            (8, "🌱 Fiches de référence", "Une fiche pratique par culture",
             "Idéal pour un débutant : tout y est expliqué."),
            (9, "🚜 Plan de la ferme", "Décrire vos parcelles",
             "À faire une première fois, puis à ajuster."),
            (10, "🗺️ Carte régionale", "Climat et sol de votre région",
        ...
        self._rafraichir_resume_semaine_accueil()

    def _rafraichir_resume_semaine_accueil(self):
        """Remplit le résumé "Cette semaine" de la page d'accueil (même
        logique que la boîte "Semaine en cours" de l'onglet Planning
        cultural et que l'onglet Semaine, voir _lignes_pour_semaine)."""
        if not hasattr(self, "table_accueil_semaine"):
            return
        semaine = semaine_actuelle()
        lignes = self._lignes_pour_semaine(semaine)

        self.table_accueil_semaine.setRowCount(len(lignes))
        for row_pos, r in enumerate(lignes):
            ...
            for col, v in enumerate(valeurs):
                self.table_accueil_semaine.setItem(row_pos, col, QTableWidgetItem(str(v)))

        self.box_accueil_semaine.setTitle(
            f"🗓️ Cette semaine (semaine {semaine}) — {len(lignes)} action(s)")

    # ============================================================
    # Onglet "Semaine" (synthèse hebdomadaire : quoi semer / planter /
    # récolter / travailler cette semaine, modulé par la météo)
    # ============================================================
    def _lignes_pour_semaine(self, semaine):
        """Renvoie les lignes du planning (self.rows) dont l'intervalle
        [semaine_debut, semaine_fin] couvre `semaine`, triées par culture.
        Factorisé pour être partagé entre la boîte "Semaine en cours" de
        l'onglet Planning et l'onglet Semaine."""
        lignes = []
        for r in self.rows:
            try:
    ...
    def _build_onglet_semaine(self, parent):
        layout = QVBoxLayout(parent)

        info = QLabel(
            "Cette page synthétise, semaine par semaine, ce qu'il y a à faire d'après le "
            "planning cultural (semis, plantation, récolte, travail du sol...), et indique "
            "si la météo de la semaine est favorable à chaque action (le mode « Prévisions » "
            "n'est fiable que sur 3 jours, « Tendances » sur 10 jours : au-delà, seul le "
            "planning est affiché)."
        )
        ...
        ligne_haut.addWidget(QLabel("Semaine (1-52) :"))
        self.spin_semaine_synthese = QSpinBox()
        self.spin_semaine_synthese.setRange(1, 52)
        self.spin_semaine_synthese.valueChanged.connect(self._rafraichir_synthese_semaine)
        ligne_haut.addWidget(self.spin_semaine_synthese)
        btn_synth_semaine_actuelle = QPushButton("📍 Semaine actuelle")
        btn_synth_semaine_actuelle.clicked.connect(
            lambda: self.spin_semaine_synthese.setValue(semaine_actuelle()))
        ligne_haut.addWidget(btn_synth_semaine_actuelle)
        ligne_haut.addStretch(1)
        layout.addLayout(ligne_haut)

        # --- Paramètres météo ---
        box_meteo = QGroupBox("🌦 Météo de la semaine")
        form_meteo = QFormLayout(box_meteo)

        ligne_ville = QHBoxLayout()
        self.edit_synth_ville = QLineEdit("Paris (75000)")
        ligne_ville.addWidget(self.edit_synth_ville)
        btn_synth_localiser = QPushButton("📍")
        btn_synth_localiser.setMaximumWidth(36)
        btn_synth_localiser.setToolTip("Choisir sur une carte ou par coordonnées GPS")
        btn_synth_localiser.clicked.connect(lambda: self._choisir_ville(self.edit_synth_ville))
        ligne_ville.addWidget(btn_synth_localiser)
        form_meteo.addRow("Ville (Météociel) :", ligne_ville)

        ligne_mode = QHBoxLayout()
        self.cb_synth_mode = QComboBox()
        self.cb_synth_mode.addItems(list(md.MODES_DISPONIBLES.keys()))
        self.cb_synth_mode.currentIndexChanged.connect(self._maj_etat_modele_synthese)
        ligne_mode.addWidget(self.cb_synth_mode, 2)
        ligne_mode.addWidget(QLabel("Modèle :"))
        self.cb_synth_modele = QComboBox()
        self.cb_synth_modele.addItems(md.MODELES_DISPONIBLES)
        ligne_mode.addWidget(self.cb_synth_modele)
        form_meteo.addRow("Source météo :", ligne_mode)

        self.btn_synth_analyser = QPushButton("🔍 Analyser la météo de la semaine")
        self.btn_synth_analyser.setStyleSheet(f"font-weight: bold; color: white; background-color: {COULEUR_ACCENT};")
        self.btn_synth_analyser.setMinimumHeight(30)
        self.btn_synth_analyser.clicked.connect(self._analyser_meteo_semaine)
        form_meteo.addRow(self.btn_synth_analyser)

        layout.addWidget(box_meteo)

        # --- Une zone (groupbox) par catégorie d'actions ---
        self._synth_categories = []  # [{"actions": [...], "box": QGroupBox, "liste": QListWidget, "lbl_meteo": QLabel}]
        zone_scroll = QScrollArea()
        zone_scroll.setWidgetResizable(True)
        contenu = QWidget()
        layout_contenu = QVBoxLayout(contenu)

        for titre, actions in md.CATEGORIES_SEMAINE:
            box = QGroupBox(titre)
            layout_box = QVBoxLayout(box)

            liste = QListWidget()
            liste.setMaximumHeight(110)
            layout_box.addWidget(liste)

            lbl_meteo = QLabel("Météo : cliquez sur « Analyser la météo de la semaine » ci-dessus.")
            lbl_meteo.setWordWrap(True)
            lbl_meteo.setStyleSheet(f"color: {COULEUR_INFO}; font-style: italic;")
            layout_box.addWidget(lbl_meteo)

            layout_contenu.addWidget(box)
            self._synth_categories.append(
                {"actions": actions, "box": box, "liste": liste, "lbl_meteo": lbl_meteo})

        # Catégorie "filet de sécurité" : toute ligne dont l'action ne
        # correspond à aucune catégorie ci-dessus (ex. anciennes actions
        # supprimées du planning comme "Conservation") reste visible ici
        # plutôt que de disparaître silencieusement de la synthèse.
        box_autres = QGroupBox("🗄️ Autres actions (non catégorisées)")
        box_autres.setToolTip(
            "Actions présentes dans votre planning mais qui ne correspondent plus à une "
            "catégorie active (par exemple d'anciennes entrées « Conservation »).")
        layout_box_autres = QVBoxLayout(box_autres)
        liste_autres = QListWidget()
        liste_autres.setMaximumHeight(110)
        layout_box_autres.addWidget(liste_autres)
        lbl_meteo_autres = QLabel("Non analysée par la météo (action non reconnue).")
        lbl_meteo_autres.setWordWrap(True)
        lbl_meteo_autres.setStyleSheet(f"color: {COULEUR_INFO}; font-style: italic;")
        layout_box_autres.addWidget(lbl_meteo_autres)
        layout_contenu.addWidget(box_autres)
        self._synth_categories.append(
            {"actions": None, "box": box_autres, "liste": liste_autres, "lbl_meteo": lbl_meteo_autres})

        layout_contenu.addStretch(1)
        zone_scroll.setWidget(contenu)
        layout.addWidget(zone_scroll, 1)

        self.spin_semaine_synthese.setValue(semaine_actuelle())
        self._rafraichir_synthese_semaine()

    def _maj_etat_modele_synthese(self):
        mode = md.MODES_DISPONIBLES.get(self.cb_synth_mode.currentText(), "forecasts")
        self.cb_synth_modele.setEnabled(mode == "forecasts")

    def _rafraichir_synthese_semaine(self):
        """Remplit chaque catégorie avec les lignes du planning concernées
        par la semaine sélectionnée (partie locale, sans réseau)."""
        if not hasattr(self, "_synth_categories"):
            return
        semaine = self.spin_semaine_synthese.value()
        lignes = self._lignes_pour_semaine(semaine)

        toutes_actions_categorisees = {a for _, acts in md.CATEGORIES_SEMAINE for a in acts}

        for cat in self._synth_categories:
            cat["liste"].clear()
            if cat["actions"] is None:
                lignes_cat = [r for r in lignes if r.get("action") not in toutes_actions_categorisees]
                if not lignes_cat:
                    cat["box"].setVisible(False)
                    continue
                cat["box"].setVisible(True)
                for r in lignes_cat:
                    texte = (f"{r.get('culture', '')} ({r.get('conduite', '')}) — "
                             f"{r.get('action', '') or '(action vide)'} "
                             f"— sem. {r.get('semaine_debut', '')}-{r.get('semaine_fin', '')}")
                    if r.get("commentaire"):
                        texte += f" — {r['commentaire']}"
                    cat["liste"].addItem(QListWidgetItem(texte))
                continue

            cat["lbl_meteo"].setText("Météo : cliquez sur « Analyser la météo de la semaine » ci-dessus.")
            cat["lbl_meteo"].setStyleSheet(f"color: {COULEUR_INFO}; font-style: italic;")
            lignes_cat = [r for r in lignes if r.get("action") in cat["actions"]]
            if not lignes_cat:
                item = QListWidgetItem("— Rien de prévu cette semaine —")
                item.setFlags(Qt.NoItemFlags)
                cat["liste"].addItem(item)
                continue
            for r in lignes_cat:
                texte = (f"{r.get('culture', '')} ({r.get('conduite', '')}) — {r.get('action', '')} "
                         f"— sem. {r.get('semaine_debut', '')}-{r.get('semaine_fin', '')}")
                if r.get("commentaire"):
                    texte += f" — {r['commentaire']}"
                cat["liste"].addItem(QListWidgetItem(texte))

        titre_semaine = (f"semaine {semaine} (en cours)" if semaine == semaine_actuelle()
                          else f"semaine {semaine}")
        self._set_status(f"Synthèse hebdomadaire : {titre_semaine}, {len(lignes)} action(s) au planning.")

    def _analyser_meteo_semaine(self):
        if not hasattr(self, "_synth_categories"):
            return
        semaine = self.spin_semaine_synthese.value()
        lignes = self._lignes_pour_semaine(semaine)
        ville = self.edit_synth_ville.text().strip()
        if not ville:
            QMessageBox.warning(self, "Champ requis", "Veuillez indiquer une ville pour la météo.")
            return

        actions_a_evaluer = sorted({
            r.get("action") for r in lignes
            if r.get("action") in md.CRITERES_ACTION
        })
        if not actions_a_evaluer:
            for cat in self._synth_categories:
                if cat["actions"] is not None:
                    cat["lbl_meteo"].setText("Météo : aucune action prévue cette semaine dans cette catégorie.")
            return

        mode = md.MODES_DISPONIBLES.get(self.cb_synth_mode.currentText(), "forecasts")
        modele = self.cb_synth_modele.currentText()

        for cat in self._synth_categories:
            if cat["actions"] and any(a in cat["actions"] for a in actions_a_evaluer):
                cat["lbl_meteo"].setText("⏳ Récupération des prévisions météo Météociel en cours...")
        self.btn_synth_analyser.setEnabled(False)
        self._set_status("Analyse météo de la semaine en cours...")

        thread = WorkerThread(md.recommander_multi, actions_a_evaluer, ville, mode=mode, modele=modele)
        thread.succes.connect(self._synthese_semaine_terminee)
        thread.echec.connect(self._synthese_semaine_echec)
        thread.finished.connect(lambda: self._nettoyer_thread(thread))
        self._threads.append(thread)
        thread.start()

    def _synthese_semaine_terminee(self, resultats):
        self.btn_synth_analyser.setEnabled(True)
        for cat in self._synth_categories:
            if cat["actions"] is None:
                continue
            actions_presentes = [a for a in cat["actions"] if a in resultats]
            if not actions_presentes:
                continue
            lignes_verdict = []
            for action in actions_presentes:
                _, meilleur, _ = resultats[action]
                lignes_verdict.append(f"{action} — {md.verdict_court(meilleur)}")
            cat["lbl_meteo"].setText("Météo :\n" + "\n".join(lignes_verdict))
            cat["lbl_meteo"].setStyleSheet("")
        self._set_status("Analyse météo de la semaine terminée.")

    def _synthese_semaine_echec(self, erreur):
        self.btn_synth_analyser.setEnabled(True)
        for cat in self._synth_categories:
            if cat["lbl_meteo"].text().startswith("⏳"):
                cat["lbl_meteo"].setText(f"❌ Impossible de récupérer la météo : {erreur}")
                cat["lbl_meteo"].setStyleSheet(f"color: {COULEUR_ALERTE};")
    ...
    def _build_onglet_planning(self, parent):
        ...
        # --- Ouverture d'un autre CSV directement depuis l'onglet ---
        ligne_csv = QHBoxLayout()
        btn_charger_csv = QPushButton("📂 Charger un autre CSV...")
        btn_charger_csv.setToolTip(
            "Ouvre un autre fichier CSV et remplace le planning actuellement affiché.")
        btn_charger_csv.clicked.connect(self.ouvrir_csv)
        ligne_csv.addWidget(btn_charger_csv)
        ligne_csv.addStretch(1)
        layout.addLayout(ligne_csv)
        ...
        # Le tableau est désormais toujours regroupé par culture puis trié
        # par enchaînement agronomique des actions (voir _rafraichir_tableau
        # et GROUPES_ACTIONS) : le tri manuel par clic sur une colonne est
        # donc retiré, il serait immédiatement écrasé par ce regroupement.
    ...
    def _load_csv(self, path, silent=False):
        ...
        if os.path.exists(path):
            try:
                nb_conservation_ignorees = 0
                with open(path, newline="", encoding="utf-8") as f:
                    reader = csv.DictReader(f, delimiter=";")
                    for r in reader:
                        # L'action "Conservation" n'existe plus dans le
                        # planning : on ignore silencieusement ces
                        # anciennes lignes plutôt que de les charger.
                        if r.get("action", "").strip() == ACTION_SUPPRIMEE_CONSERVATION:
                            nb_conservation_ignorees += 1
                            continue
                        self.rows.append({k: r.get(k, "") for k in FIELDS})
                self.csv_path = path
                message = f"{len(self.rows)} entrées chargées depuis {path}"
                if nb_conservation_ignorees:
                    message += (f" ({nb_conservation_ignorees} ancienne(s) entrée(s) "
                                f"« {ACTION_SUPPRIMEE_CONSERVATION} » ignorée(s))")
        ...
        if hasattr(self, "table_accueil_semaine"):
            self._rafraichir_resume_semaine_accueil()
        if hasattr(self, "_synth_categories"):
            self._rafraichir_synthese_semaine()
    ...
    def _cle_tri_ligne(self, r):
        """Clé de tri d'une ligne à l'intérieur de son groupe "culture" :
        d'abord l'enchaînement agronomique de l'action (voir GROUPES_ACTIONS),
        puis la semaine de début, la conduite et le n° de variété."""
        ordre_action = ORDRE_ACTIONS.get(r.get("action", ""), len(GROUPES_ACTIONS))
        try:
            semaine = int(r.get("semaine_debut", 0))
        except (ValueError, TypeError):
            semaine = 0
        return (ordre_action, semaine, r.get("conduite", ""), str(r.get("variete_n", "")))

    def _rafraichir_tableau(self):
        self._visible = self._lignes_filtrees()

        # Regroupement par culture, puis tri des actions à l'intérieur de
        # chaque groupe selon l'enchaînement agronomique logique (travail du
        # sol -> compost/paillage -> semis direct/pot -> plantation -> ...).
        groupes = {}
        for idx, r in self._visible:
            groupes.setdefault(r.get("culture", ""), []).append((idx, r))
        for lignes in groupes.values():
            lignes.sort(key=lambda item: self._cle_tri_ligne(item[1]))
        cultures_triees = sorted(groupes.keys(), key=lambda c: c.lower())

        self.table.blockSignals(True)
        self.table.setRowCount(0)
        nb_colonnes = len(FIELDS)
        for culture in cultures_triees:
            lignes = groupes[culture]
            row_pos = self.table.rowCount()
            self.table.insertRow(row_pos)
            entete = QTableWidgetItem(f"🌿 {culture or '(sans culture)'} — {len(lignes)} action(s)")
            entete.setFlags(Qt.ItemIsEnabled)  # ligne de groupe non sélectionnable
            entete.setBackground(QColor("#dfe9f3"))
            police = entete.font()
            police.setBold(True)
            entete.setFont(police)
            self.table.setItem(row_pos, 0, entete)
            self.table.setSpan(row_pos, 0, 1, nb_colonnes)
        ...
        self._set_status(
            f"{len(self._visible)} entrée(s) affichée(s) sur {len(self.rows)} au total, "
            f"regroupées par culture ({len(cultures_triees)} culture(s)).")
    ...
    def _on_select(self):
        ...
        if idx is None:
            return  # ligne d'en-tête de groupe (nom de culture), pas une entrée
        ...
        action_r = r.get("action", ACTIONS[0])
        i = self.cb_action.findText(action_r)
        if i < 0 and action_r:
            # Valeur d'action présente dans les données mais absente de la
            # liste courante (ex. ancienne action supprimée comme
            # "Conservation") : on l'ajoute temporairement à la liste pour
            # ne pas la perdre/écraser silencieusement si l'utilisateur
            # enregistre sans y toucher. On réutilise l'entrée temporaire
            # si elle a déjà été ajoutée lors d'une sélection précédente.
            label_temp = f"{action_r} (ancienne action, non reconnue)"
            i = self.cb_action.findText(label_temp)
            if i < 0:
                self.cb_action.addItem(label_temp)
                i = self.cb_action.count() - 1
    ...
    def _lire_formulaire(self):
        ...
        if not culture:
            ...
            return None
        action_texte = self.cb_action.currentText()
        suffixe = " (ancienne action, non reconnue)"
        if action_texte.endswith(suffixe):
            action_texte = action_texte[: -len(suffixe)]
        return {
            "culture": culture,
    ...
    def _creer_ligne_fiche(self, form_layout: QFormLayout, libelle: str) -> QLabel:
        ...
        lbl_libelle.setAlignment(Qt.AlignLeft | Qt.AlignTop)
        lbl_valeur = LabelChampFiche("")
        lbl_valeur.setWordWrap(True)
        lbl_valeur.setAlignment(Qt.AlignLeft | Qt.AlignTop)
        lbl_valeur.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Preferred)
    ...
    def _build_sol_fertilisation_plan(self, parent):
        ...
        self.echelle_ferti_x = 1.0
        self.echelle_ferti_y = 1.0
    ...
    def _ferti_charger_depuis_fichier(self, path):
        try:
            _, echelle_x, echelle_y = pf.charger_plan(path, self.scene_ferti)
        except Exception as e:
            QMessageBox.critical(self, "Erreur", f"Impossible de charger le plan :\n{e}")
            return
        self.plan_ferti_path = path
        self.echelle_ferti_x = echelle_x
        self.echelle_ferti_y = echelle_y
        if echelle_x == echelle_y:
            texte_echelle = f"échelle : 1 unité = {echelle_x} m"
        else:
            texte_echelle = f"échelle : 1 unité (largeur) = {echelle_x} m, 1 unité (hauteur) = {echelle_y} m"
        self.lbl_ferti_plan.setText(f"Plan : {os.path.basename(path)}  ({texte_echelle})")
    ...
    def _ferti_recalculer(self):
        ...
        echelle_x = getattr(self, "echelle_ferti_x", 1.0)
        echelle_y = getattr(self, "echelle_ferti_y", echelle_x)
        self._ferti_resultats = {}

        for item in planches:
            nom_zone = self._ferti_zone_ancetre(item)
            surface = pf.surface_m2(item.rect().width(), item.rect().height(), echelle_x, echelle_y)
    ...
    def _build_onglet_plan_ferme(self, parent):
        ...
        # --- Échelle du plan (largeur/longueur et hauteur, éventuellement liées) ---
        barre_echelle = QHBoxLayout()
        barre_echelle.addWidget(QLabel("Échelle :  1 unité de dessin en largeur/longueur ="))
        self.spin_echelle_ferme_x = QDoubleSpinBox()
        self.spin_echelle_ferme_x.setRange(0.001, 1000.0)
        self.spin_echelle_ferme_x.setDecimals(3)
        self.spin_echelle_ferme_x.setSingleStep(0.1)
        self.spin_echelle_ferme_x.setValue(1.0)
        self.spin_echelle_ferme_x.setSuffix(" m")
        self.spin_echelle_ferme_x.setToolTip(
        ...
        barre_echelle.addWidget(self.spin_echelle_ferme_x)

        barre_echelle.addWidget(QLabel("   1 unité de dessin en hauteur ="))
        self.spin_echelle_ferme_y = QDoubleSpinBox()
        self.spin_echelle_ferme_y.setRange(0.001, 1000.0)
        self.spin_echelle_ferme_y.setDecimals(3)
        self.spin_echelle_ferme_y.setSingleStep(0.1)
        self.spin_echelle_ferme_y.setValue(1.0)
        self.spin_echelle_ferme_y.setSuffix(" m")
        self.spin_echelle_ferme_y.setEnabled(False)
        barre_echelle.addWidget(self.spin_echelle_ferme_y)

        self.chk_echelle_liee = QCheckBox("Même échelle dans les deux sens")
        self.chk_echelle_liee.setChecked(True)
        self.chk_echelle_liee.setToolTip(
            "Décochez pour régler séparément l'échelle en largeur/longueur et en hauteur "
            "(utile si le plan n'a pas été dessiné au même nombre d'unités par mètre dans "
            "les deux sens). Par défaut : 1 unité de dessin = 1 mètre, dans les deux sens.")
        self.chk_echelle_liee.toggled.connect(self._pf_basculer_echelle_liee)
        self.spin_echelle_ferme_x.valueChanged.connect(self._pf_synchroniser_echelle_liee)
        barre_echelle.addWidget(self.chk_echelle_liee)
        ...
        # --- Zone de dessin + panneau latéral (aire en temps réel pendant le tracé) ---
        zone_dessin = QHBoxLayout()
        ...
        self.vue_ferme.callback_dessin_en_cours = self._pf_maj_aire_en_cours
        zone_dessin.addWidget(self.vue_ferme, 1)

        # Panneau latéral : affiche en temps réel les dimensions et l'aire
        # (m²) du rectangle en cours de tracé (mode "Dessiner un rectangle").
        box_aire = QGroupBox("📐 Aire en cours de tracé")
        box_aire.setMaximumWidth(220)
        layout_aire = QVBoxLayout(box_aire)
        self.lbl_pf_aire = QLabel()
        self.lbl_pf_aire.setWordWrap(True)
        self.lbl_pf_aire.setTextFormat(Qt.RichText)
        layout_aire.addWidget(self.lbl_pf_aire)
        layout_aire.addStretch(1)
        zone_dessin.addWidget(box_aire)

        layout.addLayout(zone_dessin, 1)
        self._pf_maj_aire_en_cours(None, None)  # texte initial (aucun tracé en cours)
    ...
    def _pf_maj_aire_en_cours(self, largeur_unites, hauteur_unites):
        """Appelée en continu par VueFerme pendant le tracé d'un rectangle
        (mode "Dessiner un rectangle") avec ses dimensions courantes en
        unités de dessin, pour afficher dimensions et aire réelle (m²) en
        temps réel dans le panneau latéral. Appelée avec (None, None) quand
        aucun tracé n'est en cours (au repos, ou une fois le tracé terminé)."""
        if largeur_unites is None or hauteur_unites is None:
            self.lbl_pf_aire.setText(
                "<i>Passez en mode « ▭ Dessiner un rectangle », puis cliquez-glissez sur "
                "le plan : les dimensions et l'aire s'affichent ici en temps réel.</i>")
            return
        echelle_x, echelle_y = self._pf_echelles()
        largeur_m = largeur_unites * echelle_x
        hauteur_m = hauteur_unites * echelle_y
        aire_m2 = pf.surface_m2(largeur_unites, hauteur_unites, echelle_x, echelle_y)
        self.lbl_pf_aire.setText(
            f"<b>Largeur :</b> {largeur_m:.2f} m<br>"
            f"<b>Hauteur :</b> {hauteur_m:.2f} m<br>"
            f"<b style='color:{COULEUR_ACCENT};font-size:14px;'>Aire : {aire_m2:.2f} m²</b>")

    def _pf_echelles(self):
        """Renvoie le tuple (echelle_x, echelle_y) actuellement réglé dans
        l'onglet Plan de la ferme (mètres réels par unité de dessin, en
        largeur/longueur puis en hauteur)."""
        return self.spin_echelle_ferme_x.value(), self.spin_echelle_ferme_y.value()

    def _pf_basculer_echelle_liee(self, liee):
        self.spin_echelle_ferme_y.setEnabled(not liee)
        if liee:
            self.spin_echelle_ferme_y.setValue(self.spin_echelle_ferme_x.value())

    def _pf_synchroniser_echelle_liee(self, valeur):
        if self.chk_echelle_liee.isChecked():
            self.spin_echelle_ferme_y.setValue(valeur)
    ...
    def _pf_dessiner_plusieurs(self):
        ...
        dialog = pf.DialogRectsMultiples(self, cultures=self._pf_liste_cultures(),
                                          echelle_x=echelle_x, echelle_y=echelle_y)
        if dialog.exec_() == dialog.Accepted:
    ...
    def _pf_charger_plan(self):
        ...
        if not path:
            return
        try:
            prochain_id, echelle_x, echelle_y = pf.charger_plan(path, self.scene_ferme)
            self.vue_ferme._prochain_id = prochain_id
            self.chk_echelle_liee.setChecked(echelle_x == echelle_y)
            self.spin_echelle_ferme_x.setValue(echelle_x)
            self.spin_echelle_ferme_y.setValue(echelle_y)
            self.plan_ferme_path = path
            self.vue_ferme.ajuster_a_la_vue()
            if echelle_x == echelle_y:
                texte_echelle = f"échelle : 1 unité = {echelle_x} m"
            else:
                texte_echelle = (f"échelle : 1 unité (largeur) = {echelle_x} m, "
                                  f"1 unité (hauteur) = {echelle_y} m")
            self._set_status(f"Plan chargé : {path} ({texte_echelle})")
        except Exception as e:
            QMessageBox.critical(self, "Erreur", f"Impossible de charger le plan :\n{e}")
    ...
    def _pf_enregistrer_plan(self):
        ...
        try:
            echelle_x, echelle_y = self._pf_echelles()
            pf.sauvegarder_plan(self.plan_ferme_path, self.scene_ferme, echelle_x, echelle_y)
