...
class VueFerme(QGraphicsView):
    ...
    def __init__(self, scene, parent=None):
        ...
        # Appelé en continu pendant le tracé d'un rectangle (mode "draw"),
        # avec (largeur, hauteur) en unités de dessin de la prévisualisation
        # courante, pour permettre d'afficher l'aire en temps réel dans un
        # panneau latéral. Appelé avec (None, None) quand le tracé se
        # termine (validé ou annulé), pour effacer cet affichage.
        self.callback_dessin_en_cours = None
    ...
    def mousePressEvent(self, event):
        if self.mode == "draw" and event.button() == Qt.LeftButton:
            ...
            if self.callback_dessin_en_cours:
                self.callback_dessin_en_cours(0.0, 0.0)
            return
    ...
    def mouseMoveEvent(self, event):
        if self.mode == "draw" and self._rect_previsualisation is not None:
            ...
            rect = QRectF(self._debut_dessin, fin).normalized()
            self._rect_previsualisation.setRect(rect)
            if self.callback_dessin_en_cours:
                self.callback_dessin_en_cours(rect.width(), rect.height())
            return
    ...
    def mouseReleaseEvent(self, event):
        if self.mode == "draw" and self._rect_previsualisation is not None:
            ...
            if self.callback_dessin_en_cours:
                self.callback_dessin_en_cours(None, None)
            self.definir_mode("select")
            return
