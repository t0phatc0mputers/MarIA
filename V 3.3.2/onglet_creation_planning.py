...
def construire_gabarit_gantt(lignes):
    ...
    culture/semaine_debut/semaine_fin), construit :
      ...
      - blocs : dict {(zone,chapelle,planche): [(texte, sem_debut, sem_fin), ...]}
    ...
    Une planche n'étant assignée qu'à une seule culture par l'assistant
    (les différents "variete_n" d'une même culture ne sont que des lots de
    semis successifs, pas des cultures distinctes), les lignes sont ici
    regroupées par planche+culture (toutes leurs actions confondues, du
    travail du sol à la récolte) pour produire un seul bloc englobant par
    planche - qui ne peut donc jamais se chevaucher avec lui-même à
    l'affichage.
    ...
    for l in lignes:
        ...
        if not (str(sd).isdigit() and str(sf).isdigit()):
            continue
        sous_cle = l.get("culture", "")
        d, f = int(sd), int(sf)
        courant = groupes.setdefault(cle, {}).setdefault(sous_cle, [d, f])
        courant[0] = min(courant[0], d)
    ...
    blocs = {}
    for cle, sous_groupes in groupes.items():
        liste = []
        for texte, (d, f) in sous_groupes.items():
            if d <= f:
                liste.append((texte, d, f))
            else:
                # chevauche le changement d'année : deux blocs distincts
                liste.append((texte, d, 52))
                liste.append((texte, 1, f))
        blocs[cle] = liste
...
class DialogueVueGantt(QDialog):
    ...
    def _construire(self):
        ...
        for idx, cle in enumerate(planches_ordre):
            ...
            for a in range(nb_annees):
                offset = a * 52
                for texte, d, f in blocs.get(cle, []):
                    item = QTableWidgetItem(texte)
