...
def construire_gabarit_gantt(lignes):
    ...
    Chaque étape (planche + année + culture) est ici regroupée sur toutes
    ses actions confondues (du travail du sol à la récolte) pour produire
    un seul bloc englobant par étape - ce qui permet à une même planche
    d'afficher des cultures différentes selon l'année, au fil d'une longue
    période (rotation étalée sur plusieurs saisons).
    ...
    for cle, sous_groupes in groupes.items():
        liste = []
        for (annee, texte), (d, f) in sous_groupes.items():
            if d <= f:
...
def exporter_gantt_vers_excel(chemin_xlsx, lignes, annee_depart=None, nb_annees=None,
    ...
    l'application. Nécessite openpyxl. Renvoie (annee_depart, nb_annees)
...
class DialogueVueGantt(QDialog):
    ...
    def _exporter_excel(self):
        ...
        QMessageBox.information(self, "Export réussi",
                                 f"La vue Gantt a été exportée vers :\n{chemin}")
