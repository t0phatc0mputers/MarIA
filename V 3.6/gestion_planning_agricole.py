...
# secondes. QtWebEngineWidgets fait exception : il est importé à part (voir
# _importer_webengine), tout de suite après la création de QApplication et
# AVANT l'affichage de l'écran de chargement - contrainte macOS, sinon
# segmentation fault. Les noms sont déclarés ici à None/False pour que le
# reste du fichier puisse s'y référer sans erreur avant que l'import
# effectif n'ait eu lieu.
...
def _importer_webengine():
    """Importe QtWebEngineWidgets (utilisé par la carte interactive).

    Ce module DOIT être importé juste après la création de QApplication et
    avant tout autre traitement d'évènements Qt (splash, processEvents...) :
    sur macOS, importer QtWebEngineWidgets plus tard - par exemple après
    avoir déjà affiché une fenêtre de chargement et pompé la boucle
    d'évènements - provoque un crash immédiat (segmentation fault) lié à
    l'initialisation du contexte OpenGL partagé du moteur Chromium embarqué.
    Le réglage Qt.AA_ShareOpenGLContexts (positionné avant la création de
    QApplication) ne suffit pas à lui seul à éviter ce crash sur macOS."""
    global QWebEngineView, WEBENGINE_DISPONIBLE
    try:
...
def _importer_modules_lourds(splash=None):
    ...
    imports. QtWebEngineWidgets n'est PAS importé ici : voir
    _importer_webengine, qui doit être appelée avant, dès la création de
    QApplication (contrainte macOS)."""
...
class DialogueProfilSol(QDialog):
    ...
    def __init__(self, parent=None, valeurs=None, zones=None):
        ...
        self.cb_produit = QComboBox()
        for e in asol.ENGRAIS_AMENDEMENTS:
            self.cb_produit.addItem(e["nom"])
        i = self.cb_produit.findText(valeurs.get("produit_nom", ""))
        self.cb_produit.setCurrentIndex(max(0, i))
        form.addRow("Produit à épandre :", self.cb_produit)
        ...
        note = QLabel(
            ...
            "de la culture est utilisé sans correctif (comme un Bilan Simple).")
    ...
    def valeurs(self):
        ...
        return {
            ...
            "zone": zone_texte,
            "produit_nom": self.cb_produit.currentText(),
...
class VueFermeFertilisation(QGraphicsView):
    ...
    def ajuster_a_la_vue(self):
        ...
        nécessaire ici aussi pour les plans en dimensions réelles (mètres)."""
        rect = self.scene().itemsBoundingRect()
        if rect.isEmpty():
            return
        marge = max(rect.width(), rect.height()) * 0.05 or 1.0
        rect = rect.adjusted(-marge, -marge, marge, marge)
...
class PlanningApp(QMainWindow):
    ...
    def _build_sol_fertilisation_plan(self, parent):
        ...
        splitter.addWidget(self.txt_ferti_details)
        splitter.setStretchFactor(0, 3)
        splitter.setStretchFactor(1, 1)
    ...
    def _ferti_rafraichir_liste_profils(self):
        ...
        for p in self.profils_sol:
            ...
            item = QListWidgetItem(f"{p['nom']} ({zone_txt}, {p['produit_nom']})")
    ...
    def _ferti_recalculer(self):
        ...
        for item in planches:
            ...
            produit = None
            if profil:
                produit = next((e for e in asol.ENGRAIS_AMENDEMENTS
                                 if e["nom"] == profil["produit_nom"]), None)
...
def main():
    # Positionné avant la création de QApplication (obligatoire pour que
    # QWebEngineView fonctionne s'il est utilisé), et sans condition ici
    # puisqu'on ne sait pas encore, à ce stade, si le module QtWebEngine
    # sera disponible : son import est différé après l'affichage de la
    # fenêtre de chargement (voir plus bas). Ce réglage est sans effet
    # néfaste si la carte interactive n'est finalement pas utilisée.
    ...
    # Importé tout de suite après QApplication, avant le moindre
    # processEvents() : voir la note dans _importer_webengine (contrainte
    # macOS - sinon segmentation fault).
    _importer_webengine()
