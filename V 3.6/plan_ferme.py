...
class VueFerme(QGraphicsView):
    ...
    def __init__(self, scene, parent=None):
        ...
        # de sélection multiple (rubber band) : la sélection multiple par
        # cadre n'est utilisée nulle part ailleurs dans l'application,
        # alors que se déplacer sur un plan qui peut désormais s'étendre
        # sur plusieurs centaines de mètres (dimensions réelles) est
        # nécessaire. Cliquer-glisser directement sur un rectangle continue
    ...
    def ajuster_a_la_vue(self):
        """Recentre et redimensionne automatiquement le zoom pour que
        l'ensemble du plan tienne dans la fenêtre visible. Indispensable
        après le chargement d'un plan en dimensions réelles (mètres) : sans
        cela, des planches de 1,30 m de large s'affichent à peine plus
        épaisses qu'un trait, quasiment invisibles à l'échelle 1 unité = 1
        pixel utilisée par défaut par QGraphicsView."""
