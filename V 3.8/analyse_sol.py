...
def rechercher_dose_plafond_n(texte):
    ...
    t = texte.strip().lower().rstrip("s")
    def correspond(champ):
        c = (champ or "").strip().lower()
        if not c:
            return False
        return t in c or c.rstrip("s") in t
    return [p for p in DOSE_PLAFOND_N if correspond(p["legume"]) or correspond(p["variante"])]
...
# ====================================================================
# 5. FERTILISATION PAR PLANCHE (à partir du plan de la ferme)
#    Combine CULTURES_BILAN_AZOTE (besoin brut de la culture assignée à
#    la planche), un correctif simple selon la richesse du sol (si un
#    profil d'analyse est disponible pour la zone) et ENGRAIS_AMENDEMENTS
#    (produit choisi), pour obtenir une dose de produit sur la surface
#    réelle de la planche. Niveau "Bilan Simple" du classeur (pas de
#    reliquats/minéralisation, qui demanderaient une analyse plus fine
#    que ce qu'un profil de zone peut raisonnablement fournir).
# ====================================================================
COEFFICIENT_PAR_CATEGORIE_TENEUR = {
    "< Trenf (renforcement nécessaire)": 1.3,
    "Trenf – Timp -10 %": 1.1,
    "Timp -10 % – Timp": 1.0,
    "Timp – Timp +10 % (impasse possible)": 0.9,
    "Timp +10 % – 2 x Timp": 0.6,
    "2 x Timp – 3 x Timp": 0.3,
    "> 3 x Timp (excès)": 0.0,
}


def meilleur_produit_pour_besoin(besoins_kg_ha, produits=None):
    """Choisit, parmi ENGRAIS_AMENDEMENTS (ou `produits` fourni), le produit
    dont le ratio N/P2O5/K2O se rapproche le plus du besoin donné (kg/ha) —
    similarité cosinus entre le vecteur besoin et le vecteur du produit
    (proportions relatives des 3 éléments, indépendamment de leur
    concentration absolue) : un produit riche en azote sera préféré pour
    une culture gourmande en azote, un produit équilibré pour une culture
    aux besoins équilibrés, etc. Renvoie le premier produit de la liste si
    le besoin est nul sur les 3 éléments (rien à départager)."""
    produits = produits if produits is not None else ENGRAIS_AMENDEMENTS
    if not produits:
        return None
    vecteur_besoin = [besoins_kg_ha.get("N", 0) or 0, besoins_kg_ha.get("P2O5", 0) or 0,
                       besoins_kg_ha.get("K2O", 0) or 0]
    norme_besoin = sum(v * v for v in vecteur_besoin) ** 0.5
    if norme_besoin == 0:
        return produits[0]

    meilleur, meilleure_similarite = None, -1.0
    for p in produits:
        vecteur_prod = [p.get("N_kg_t") or 0, p.get("P2O5_kg_t") or 0, p.get("K2O_kg_t") or 0]
        norme_prod = sum(v * v for v in vecteur_prod) ** 0.5
        if norme_prod == 0:
            continue
        produit_scalaire = sum(a * b for a, b in zip(vecteur_besoin, vecteur_prod))
        similarite = produit_scalaire / (norme_besoin * norme_prod)
        if similarite > meilleure_similarite:
            meilleure_similarite, meilleur = similarite, p
    return meilleur or produits[0]


def calculer_besoin_planche(culture, surface_m2, produit=None, profil_sol=None):
    """Calcule le besoin de fertilisation d'une planche.

    - culture : nom recherché dans CULTURES_BILAN_AZOTE (espèce ou
      famille) ; renvoie None si aucune correspondance.
    - surface_m2 : surface réelle de la planche (voir plan_ferme.surface_m2).
    - produit : entrée de ENGRAIS_AMENDEMENTS (dict) à utiliser de force
      (imposé par l'utilisateur), ou None pour laisser
      meilleur_produit_pour_besoin choisir automatiquement le produit dont
      le ratio N/P2O5/K2O colle le mieux au besoin de la culture.
    - profil_sol : dict de valeurs de laboratoire (mêmes clés que
      analyser_parcelle : p2o5_ppm, trenf_p2o5, timp_p2o5, k2o_ppm,
      trenf_k2o, timp_k2o...), optionnel. S'il est fourni, le besoin brut
      en P2O5/K2O est corrigé selon la richesse déjà présente dans le sol
      (comme le fait le Bilan Intermédiaire du classeur, sans coefficient
      sur l'azote) ; sinon, le besoin brut de la culture est utilisé tel
      quel (comme le Bilan Simple).

    Renvoie un dict {culture, produit, produit_auto, surface_m2,
    besoins_bruts_kg_ha, besoins_corriges_kg_ha, categories_sol,
    doses_par_nutriment, dose_produit_kg} ou None si la culture n'est pas
    reconnue. `produit_auto` indique si le produit a été choisi
    automatiquement (True) ou imposé via le paramètre `produit` (False)."""
    correspondances = rechercher_cultures_bilan(culture)
    if not correspondances:
        return None
    fiche = correspondances[0]
    besoins = {"N": fiche.get("N") or 0, "P2O5": fiche.get("P") or 0, "K2O": fiche.get("K") or 0}

    coef_sol = {"P2O5": 1.0, "K2O": 1.0}
    categories = {"P2O5": None, "K2O": None}
    if profil_sol:
        cat_p = categorie_teneur(profil_sol.get("p2o5_ppm"), profil_sol.get("trenf_p2o5"),
                                  profil_sol.get("timp_p2o5"))
        cat_k = categorie_teneur(profil_sol.get("k2o_ppm"), profil_sol.get("trenf_k2o"),
                                  profil_sol.get("timp_k2o"))
        categories = {"P2O5": cat_p, "K2O": cat_k}
        if cat_p in COEFFICIENT_PAR_CATEGORIE_TENEUR:
            coef_sol["P2O5"] = COEFFICIENT_PAR_CATEGORIE_TENEUR[cat_p]
        if cat_k in COEFFICIENT_PAR_CATEGORIE_TENEUR:
            coef_sol["K2O"] = COEFFICIENT_PAR_CATEGORIE_TENEUR[cat_k]

    besoins_corriges = {
        "N": besoins["N"],
        "P2O5": besoins["P2O5"] * coef_sol["P2O5"],
        "K2O": besoins["K2O"] * coef_sol["K2O"],
    }

    produit_auto = produit is None
    produit = produit or meilleur_produit_pour_besoin(besoins_corriges)
    produit_pct = {"N": (produit.get("N_kg_t") or 0) / 10,
                   "P2O5": (produit.get("P2O5_kg_t") or 0) / 10,
                   "K2O": (produit.get("K2O_kg_t") or 0) / 10}

    doses = bilan_simple(besoins_corriges, surface_m2, produit_pct)
    # Dose pragmatique de produit à épandre sur la planche : on ne peut
    # épandre qu'UNE seule dose du produit choisi (pas une par nutriment),
    # donc on retient le nutriment le plus limitant (celui qui demande la
    # dose de produit la plus importante) - pratique courante en
    # maraîchage biologique, quitte à sur-approvisionner légèrement les
    # deux autres.
    doses_valides = [b["dose_produit_kg_surface"] for b in doses.values()
                      if b["dose_produit_kg_surface"] is not None]
    dose_produit_kg = max(doses_valides) if doses_valides else None

    return {
        "culture": fiche["espece"], "produit": produit["nom"], "produit_auto": produit_auto,
        "surface_m2": surface_m2,
        "besoins_bruts_kg_ha": besoins, "besoins_corriges_kg_ha": besoins_corriges,
        "categories_sol": categories, "doses_par_nutriment": doses,
        "dose_produit_kg": dose_produit_kg,
    }
